当前最高优先修正：保留V13最相似5段真实OHLC、方向、目标与退出；把新历史路径PAPER仓位提高到单笔4%/组合12%，取消相关敞口折半。信号成立后按不变的真实TP2反算并放大每单名义仓位，使TP2扣除估计往返成本后至少30U；30U不是拒单过滤器、利润目标或上限。保留2%保护、组合风险、亏损暂停、D1及实盘关闭；完成PR、CI、部署和线上新订单经济计划验证。
