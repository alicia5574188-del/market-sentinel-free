  ...
  if (plan?.state === "PREPARED") return (plan.invalidationSignalCount ?? 0) > 0
    ...
    : plan.marketState === "BREAKOUT"
      ? (plan.breakoutSignalCount ?? 0) > 0
        ? `高质量突破实时确认 ${plan.breakoutSignalCount}/3；连续通过后实时 IOC 进场`
        : `高质量突破连续3次盘口确认；中等质量等待完整1分钟确认`
  ...
  return <main>
    ...
    <section className="markets" hidden={tab !== "brain"}>{runtime?.symbols.map((symbol) => {
      ...
      return <article className="market" key={symbol}><div className="market-title"><div><small>{symbol.replace("_", "/")}</small><h2>{marketFresh ? status : "数据恢复中"}</h2></div><strong>{marketFresh ? num(evidence?.midpoint, 5) : "—"}</strong></div>
        ...
        {position?.status === "OPEN" ? <div className="trade-levels"><div><small>实际进场</small><b>{num(position.entryPrice, 5)}</b></div><div><small>原始结构止损</small><b>{num(position.initialStop, 5)}</b></div><div><small>当前保护位</small><b>{num(position.currentStop, 5)}</b></div><div><small>当前目标</small><b>{num(position.currentTarget, 5)}</b></div></div> : intent && <div className="trade-levels"><div><small>准备进场</small><b>{num(intent.entryTrigger, 5)}</b></div><div><small>判断错误就退出</small><b>{num(intent.invalidation, 5)}</b></div><div><small>当前目标</small><b>{num(intent.target, 5)}</b></div><div><small>{intent.economicTarget != null && intent.economicTarget !== intent.target ? "分段空间盈亏比" : "扣成本后盈亏比"}</small><b>{num(netRr(intent.entryTrigger, intent.invalidation, intent.economicTarget ?? intent.target), 2)} : 1</b></div></div>}
        <div className="execution"><small>执行方式</small><b>{position?.status === "OPEN" ? waitReason(runtime, marketFresh, symbol) : plan?.state === "PREPARED" ? plan.marketState === "BREAKOUT" ? `内部监测 ${num(plan.entryTrigger, 5)}，不预挂；高质量约6秒确认，中等质量等收盘，0.5R外不追` : liveEnabled ? `Gate 限价挂单等待 ${num(plan.entryTrigger, 5)} · 有效至 ${time(plan.expiresAt)}` : `模拟等待实时价格到达 ${num(plan.entryTrigger, 5)} · 软变化先观察，到期自动撤销` : intent ? "方向已形成，等待系统建立进场计划" : "继续等待完整机会"}</b></div>
    ...
    <section className="panel-list" hidden={tab !== "orders"}>{!openPositions.length && !preparedPlans.length && !recentClosedPositions.length && <div className="empty"><b>当前没有模拟订单</b><p>出现合适位置后会先显示准备计划；真实订单请进入底部「实盘」。</p></div>}
      ...
      {preparedPlans.map(({ symbol, plan }) => <OrderCard key={symbol} symbol={symbol} side={plan.side} label={plan.marketState === "BREAKOUT" ? "等待确认" : "等待触发"} state={plan.marketState} notional={plan.notional} equity={runtime?.equity ?? INITIAL_EQUITY} leverage={plan.leverage} margin={plan.margin} note={(plan.invalidationSignalCount ?? 0) > 0 ? `软失效观察 ${plan.invalidationSignalCount}/2，计划仍锁定；连续确认后才撤销。有效至 ${time(plan.expiresAt)}。` : plan.marketState === "BREAKOUT" ? `内部观察不占用交易所挂单；高质量连续盘口确认 ${(plan.breakoutSignalCount ?? 0)}/3，中等质量等待分钟收盘。有效至 ${time(plan.expiresAt)}。` : `本挂单有效至 ${time(plan.expiresAt)}；短周期变化先观察，硬失效或到期才立即撤销。`} values={[[plan.marketState === "BREAKOUT" ? "突破观察位" : "触发进场", plan.entryTrigger], ["结构止损", plan.invalidation], ["当前节点", plan.target], ...(plan.economicTarget != null && plan.economicTarget !== plan.target ? [["分段可达位", plan.economicTarget] as [string, number]] : []), ["计划风险", plan.plannedRisk]]} />)}
