...
const configs=[]; const add=(family,variants)=>variants.forEach((p,n)=>configs.push({id:`${family}-${configs.filter(x=>x.family===family).length+n}`,family,...p}));
...
function sig(c,f){let d=0,s=0;
 ...
 return{d,s};}
function resolve(c,f,found,fr=FRICTION,sl=SLIP){const rows=f.rows,entryI=f.i+1;if(!rows[entryI]||rows[entryI].time!==f.t+300)return null;const d=found.d,entry=rows[entryI].open*(1+d*sl),stop=entry*(1-d*c.stop),target=entry*(1+d*c.target);let exit=entry,closed=rows[entryI].time;const max=c.hold*12;
 for(let k=0;k<max&&entryI+k<rows.length;k++){const x=rows[entryI+k];if(k&&x.time!==rows[entryI+k-1].time+300){exit=rows[entryI+k-1].close;closed=rows[entryI+k-1].time;break}const st=d>0?x.low<=stop:x.high>=stop,tg=d>0?x.high>=target:x.low<=target;if(st||tg){exit=st?stop:target;closed=x.time;break}exit=x.close;closed=x.time;}return{strategyId:c.id,family:c.family,symbol:f.s,side:d>0?'LONG':'SHORT',opened:f.t+300,closed,strength:found.s,net:d*(exit-entry)/entry-fr,friction:fr};}
...
for(const c of configs){const base=tradesFor(c),stress=tradesFor(c,STRESS,SLIP);const d=stat(base,from,DISC_END),v=stat(base,DISC_END,VAL_END),e=stat(base,VAL_END,to),ds=stat(stress,from,DISC_END);const ok=d.trades>=60&&d.net>0&&d.pf>=1.005&&ds.net>0&&v.trades>=15&&v.net>0;ranking.push({config:c,d,v,e,stressDiscovery:ds,eligible:ok,score:ok?(d.tradesPerDay+2*v.tradesPerDay):0});}
const eligible=ranking.filter(x=>x.eligible).sort((a,b)=>b.score-a.score||b.v.net-a.v.net);
...
const selected=[];let bestValTpd=0;
for(const cand of eligible){const trial=[...selected,cand],p=portfolio(trial),d=pstat(p,from,DISC_END),v=pstat(p,DISC_END,VAL_END);if(d.netPnl<=0||v.netPnl<=0||v.pf<1.005||p.maxDrawdown>.20)continue;if(v.tradesPerDay>bestValTpd+.03){selected.push(cand);bestValTpd=v.tradesPerDay;}}
const base=portfolio(selected),stress=portfolio(selected,STRESS,SLIP),adverse=portfolio(selected,FRICTION,.0005);
const topNear=ranking.filter(x=>x.d.net>0&&x.stressDiscovery.net>0).sort((a,b)=>b.v.net-a.v.net||b.v.tradesPerDay-a.v.tradesPerDay).slice(0,20).map(x=>({id:x.config.id,family:x.config.family,config:x.config,d:x.d,v:x.v,e:x.e,stressDiscovery:x.stressDiscovery}));
const familyDiagnostics=Object.fromEntries([...new Set(ranking.map(x=>x.config.family))].map(family=>{const rows=ranking.filter(x=>x.config.family===family);const positiveValidation=rows.filter(x=>x.d.net>0&&x.stressDiscovery.net>0&&x.v.net>0).sort((a,b)=>b.v.tradesPerDay-a.v.tradesPerDay);return[family,{configs:rows.length,discoveryPositive:rows.filter(x=>x.d.net>0).length,stressDiscoveryPositive:rows.filter(x=>x.stressDiscovery.net>0).length,validationPositive:positiveValidation.length,best:positiveValidation[0]?{id:positiveValidation[0].config.id,d:positiveValidation[0].d,v:positiveValidation[0].v,e:positiveValidation[0].e}:null}]}));
const evalStats=pstat(base,VAL_END,to);
const report={generatedAt:new Date().toISOString(),months:raw.months,symbols,rawCandidateConfigs:configs.length,eligibleConfigs:eligible.length,selected:selected.map(x=>({id:x.config.id,family:x.config.family,config:x.config,d:x.d,v:x.v,e:x.e})),portfolio:{discovery:pstat(base,from,DISC_END),validation:pstat(base,DISC_END,VAL_END),evaluation:evalStats,maxDrawdown:base.maxDrawdown},stress:{evaluation:pstat(stress,VAL_END,to),maxDrawdown:stress.maxDrawdown},adverse:{evaluation:pstat(adverse,VAL_END,to),maxDrawdown:adverse.maxDrawdown},familyDiagnostics,topNear,target:{maximizeFrequency:true,selectionUsesEvaluation:false,minEvaluationPf:1.01,maxDrawdown:.20},frequencyCandidate:evalStats.netPnl>0&&evalStats.pf>=1.01};
writeFileSync(OUTPUT,JSON.stringify(report,null,2)+'\n');console.log('HF_V2='+JSON.stringify({eligibleConfigs:report.eligibleConfigs,selected:report.selected.map(x=>x.id),portfolio:report.portfolio,stress:report.stress,adverse:report.adverse,familyDiagnostics:report.familyDiagnostics,topNear:report.topNear.slice(0,8),frequencyCandidate:report.frequencyCandidate}));
