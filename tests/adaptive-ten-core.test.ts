...
test("relation candidate net edge stays equal to learned cost-positive evidence regardless of health",()=>{
  ...
  assert.ok(rule&&frame);
  e.rules=[{...rule!,scope:"RECENT",status:"ACTIVE",health:.64,longNet:.0017,stopRate:.0047,livePathScore:.84,environmentFit:.69,symbols:[frame!.symbol]}];
  e.frames={[frame!.symbol]:frame!};
  const c=relationCandidates(e)[0];assert.ok(c);
  assert.equal(c!.netRate,.0017);
