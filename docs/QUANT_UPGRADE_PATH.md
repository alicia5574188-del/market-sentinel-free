...
6. Open one PR, merge only on green CI, then let the deploy gate verify the immutable asset, D1 migration, both schedulers, zero pre-cutover/old-version paper positions, active brain version, and new simulation epoch. New-epoch positions are allowed.
