...
V13五段真实K线、方向和退出保持不变。新历史路径PAPER订单单笔/组合预算提高为4%/12%；信号成立后按真实TP2距离倒算并直接放大模拟名义仓位，使TP2扣除估计往返费用后至少30U。30U不是拒单过滤器、利润目标或上限，不拉远也不截短市场目标；真实空间更大时完整保留。模拟仓位可使用流动性、波动和强平安全边界内的杠杆空间。现有持仓保持其不可变入场计划，不重置brain/epoch；D1、凭据、历史和LIVE关闭不变。当前仅为本地候选，生产仍以STATUS的最后验证记录为准。
