...
export default function ForwardDashboard({data,healthy,feedAt,error,livePanel,liveEnabled,accountPanel,memberName}:{data:View|null;healthy:boolean;feedAt:number|null;error:string|null;livePanel:ReactNode;liveEnabled:boolean;accountPanel?:ReactNode;memberName?:string}) {
  ...
  return <main className="fr-app" data-ui-version="dark-live-v1" data-record-view="compact-records-pnl-v1">
    ...
    {tab==="overview"&&<>
      ...
      <div className="fr-two"><section className="fr-section"><div className="fr-section-head"><div><small>账户结果</small><h2>只看成本后的净值</h2></div><span>含模拟成本</span></div><EquityPath data={data}/><div className="fr-three"><div><small>已实现价格损益</small><b>{signed(data?.grossPnl)} U</b></div><div><small>含退出成本浮盈</small><b>{signed(data?.floating)} U</b></div><div><small>已观测最大回撤</small><b>{fmt(data?data.maxDrawdown*100:null)}%</b></div></div></section>
...
function EquityPath({data}:{data:View|null}){const series=data?[{equity:data.initialEquity},...data.daily.map(d=>({equity:d.endEquity}))]:[];if(series.length<3)return<div className="fr-chart-empty"><span>{fmt(data?.equity)} <small>USDT</small></span><p>净值曲线随每日记录更新。</p></div>;
