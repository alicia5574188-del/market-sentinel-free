...
test("final owner/source guard can still cancel the restored REST entry before fetch",async()=>{
  const real=globalThis.fetch;let calls=0;
  globalThis.fetch=async()=>{calls++;return Response.json({id_string:"123"});};
  try{
    const client=new GateLiveClient({apiKey:"fixture-key",apiSecret:"fixture-secret",environment:"live"});
    const intent={kind:"MARKET" as const,tag:"t-ms-e-guard",size:1,contracts:1,notional:100,plannedRisk:1,leverage:10,margin:10,
      body:{contract:"BTC_USDT",size:"1",price:"0",tif:"ioc",text:"t-ms-e-guard",reduce_only:false}};
    await assert.rejects(()=>client.createEntry(intent,()=>false),GateEntryCancelledError);
