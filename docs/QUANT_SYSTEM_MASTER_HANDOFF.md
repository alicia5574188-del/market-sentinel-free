## 2026-09-06 — V13.2 持续PAPER与单图真实路径

当前历史路径PAPER取消连续/当日模拟亏损暂停以持续暴露问题，保留资金、4%/12%组合、报价、D1和LIVE关闭等硬边界；顶部将当前与最相似5段真实五分钟收盘走势叠加在一张归零线图，历史右侧仍是实际后一小时。

## 2026-09-06 — V13.1 每单30U模拟仓位已上线

V13五段真实K线、方向和退出保持不变。新历史路径PAPER订单单笔/组合预算提高为4%/12%，取消相关敞口折半，所有持仓仍统一受12%组合风险约束；信号成立后按真实TP2距离倒算并直接放大模拟名义仓位，使TP2扣除估计往返费用后至少30U。30U不是拒单过滤器、利润目标或上限，不拉远也不截短市场目标；真实空间更大时完整保留。模拟仓位可使用流动性、波动和强平安全边界内的杠杆空间。现有持仓保持其不可变入场计划，不重置brain/epoch；D1、凭据、历史和LIVE关闭不变。PR #160/#161已上线，最终main `7a7a276`、workflow `33988357115`、Worker `62a2c25b-fe04-4d1b-b0df-ba5e7d5a825a` 全绿。
...
> **2026-09-06 V13 production verified.** PR #158 / main `86578d2` / workflow `33984709869` verify, deploy and operations passed; Worker `c69ee06f-08f1-4bfc-9f63-0d93870a419f`. Migration0031 completed with V13 active, target null and zero legacy PAPER holdings. Exactly five closest independent analog episodes own vote/target/replay and expose raw 24+12 OHLC candlesticks in the top UI. Current analog PAPER risk is 1% per trade / 3% portfolio without six-symbol pre-allocation; correlated exposure halves. Executable reward/full-risk boundary is 0.8R, while the 2% stop cap and all data/quote/liquidity/account/D1/LIVE boundaries remain. Bounded 12h replay: 42 signals, 21 closed, 7 wins, +2.891R, 2 open. Browser verified all five actual-candle charts and current 1% risk display; scanner/position live 6/6 and D1 within safety lines. This is limited evidence, not a profit promise.
