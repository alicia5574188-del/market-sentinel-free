...
export async function tryOpenHte31Trade(
  ...
  const row = {
    ...
    quantity,
    leverage,
    entryTrigger: `${selected.signal.label} · ${selected.signal.reasons.join("；")} · ${sizing.leverageReason}`,
...
export async function completeHte31PostExitObservation(trade: typeof hte31Trades.$inferSelect, horizonMinutes: number, candles: Hte31Candle[], now = Date.now()) {
  ...
  if (complete) {
    const learningId = learningIdFor(trade.traderId, trade.assetRegime, trade.side);
