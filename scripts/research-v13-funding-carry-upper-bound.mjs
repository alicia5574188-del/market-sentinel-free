...
const events = [...fundingByTime.entries()].filter(([, rates]) => rates.size >= minMarkets).sort((a, b) => a[0] - b[0]);
if (events.length < 100) throw new Error(`only ${events.length} synchronized funding events`);
const splits = { discovery: new Set(months.slice(0, 6)), validation: new Set(months.slice(6, 9)),
  evaluation: new Set(months.slice(9)), full: new Set(months) };
...
function simulate(config, selectedMonths, cost) {
  let equity = 1000; let peak = equity; let maxDD = 0; let wins = 0; let grossWin = 0; let grossLoss = 0;
  let trades = 0; let turnover = 0; let fundingComponent = 0; let priceComponent = 0; let costComponent = 0; let missingCandleEvents = 0;
  const monthly = new Map(); const scopedEvents = events.filter(([t]) => selectedMonths.has(monthKey(t)));
  const firstTime = scopedEvents[0]?.[0]; const lastTime = scopedEvents.at(-1)?.[0];
  for (const [time, ratesMap] of scopedEvents) {
    const mk = monthKey(time); const ranked = [...ratesMap.entries()].sort((a, b) => a[1] - b[1]);
    if (ranked.length < config.depth * 2) continue;
    const lows = ranked.slice(0, config.depth); const highs = ranked.slice(-config.depth).reverse();
    ...
    const entryTime = time - config.preMinutes * 60; const exitTime = time + config.postMinutes * 60;
    const legs = [...lows.map(([symbol, rate]) => ({ symbol, rate, side: "LONG" })),
    ...
    for (const leg of legs) {
      ...
      eventPrice += weight * priceRet; eventFunding += weight * fundingRet;
    ...
    trades += 1; turnover += 4 * SIDE_GROSS; fundingComponent += eventFunding; priceComponent += eventPrice;
    costComponent += 2 * SIDE_GROSS * cost;
  ...
  return { trades, missingCandleEvents, days: Number(days.toFixed(2)), eventsPerDay: Number((trades / days).toFixed(3)),
    ...
    maxDrawdownPct: Number((maxDD * 100).toFixed(3)), fundingComponentPct: Number((fundingComponent * 100).toFixed(3)),
    priceComponentPct: Number((priceComponent * 100).toFixed(3)), costComponentPct: Number((costComponent * 100).toFixed(3)), monthly: monthRows };
}
...
const rows = configs.map((config) => {
  const discovery = simulate(config, splits.discovery, BASE_COST); const discoveryStress = simulate(config, splits.discovery, STRESS_COST);
  const eligible = discoveryStress.turnoverPerDay >= 4 && discoveryStress.turnoverPerDay <= 6 && discoveryStress.avgMonthPct >= 5
    && discoveryStress.profitFactor >= 1 && discoveryStress.positiveMonths >= Math.max(3, Math.floor(discoveryStress.months * 0.6));
...
const targetBand = rows.filter((row) => row.discoveryStress.turnoverPerDay >= 4 && row.discoveryStress.turnoverPerDay <= 6)
  .sort((a, b) => b.discoveryStress.avgMonthPct - a.discoveryStress.avgMonthPct || b.discoveryStress.profitFactor - a.discoveryStress.profitFactor);
const targetTurnoverBest = targetBand[0] ?? null;
const targetTurnoverDiagnostics = targetTurnoverBest ? {
  config: targetTurnoverBest.config,
  discoveryZeroCost: simulate(targetTurnoverBest.config, splits.discovery, 0),
  discoveryBase: targetTurnoverBest.discovery,
  discoveryStress: targetTurnoverBest.discoveryStress,
  validationZeroCost: simulate(targetTurnoverBest.config, splits.validation, 0),
  validationBase: simulate(targetTurnoverBest.config, splits.validation, BASE_COST),
  validationStress: simulate(targetTurnoverBest.config, splits.validation, STRESS_COST),
  evaluationZeroCost: simulate(targetTurnoverBest.config, splits.evaluation, 0),
  evaluationBase: simulate(targetTurnoverBest.config, splits.evaluation, BASE_COST),
  evaluationStress: simulate(targetTurnoverBest.config, splits.evaluation, STRESS_COST),
  fullZeroCost: simulate(targetTurnoverBest.config, splits.full, 0),
  fullBase: simulate(targetTurnoverBest.config, splits.full, BASE_COST),
  fullStress: simulate(targetTurnoverBest.config, splits.full, STRESS_COST),
} : null;
const maxTurnover = [...rows].sort((a, b) => b.discoveryStress.turnoverPerDay - a.discoveryStress.turnoverPerDay)[0];
const selected = rows[0];
const validation = simulate(selected.config, splits.validation, BASE_COST); const validationStress = simulate(selected.config, splits.validation, STRESS_COST);
const evaluation = simulate(selected.config, splits.evaluation, BASE_COST); const evaluationStress = simulate(selected.config, splits.evaluation, STRESS_COST);
const full = simulate(selected.config, splits.full, BASE_COST); const fullStress = simulate(selected.config, splits.full, STRESS_COST);
const passes = Boolean(selected.eligible && validationStress.avgMonthPct >= 5 && evaluationStress.avgMonthPct >= 5
  && validationStress.turnoverPerDay >= 4 && evaluationStress.turnoverPerDay >= 4 && validationStress.returnPct > 0 && evaluationStress.returnPct > 0);
...
const result = {
  ...
  assumptions: { sideGrossFraction: SIDE_GROSS, grossExposureDuringWindow: 2 * SIDE_GROSS, twoWayTurnoverPerFundingEvent: 4 * SIDE_GROSS,
    baseRoundTripCostPerLeg: BASE_COST, stressRoundTripCostPerLeg: STRESS_COST, targetTurnoverPerDay: 5 },
  search: { configs: configs.length, discoveryEligible: rows.filter((x) => x.eligible).length, targetBandConfigs: targetBand.length,
    maxDiscoveryTurnover: maxTurnover?.discoveryStress.turnoverPerDay ?? 0, top: rows.slice(0, 10) },
  targetTurnoverDiagnostics,
