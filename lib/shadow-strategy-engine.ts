...
function completed(candles: Candle[], intervalMs: number, observedAt: number) {
  return candles
    .filter((candle) => [candle.time, candle.open, candle.high, candle.low, candle.close, candle.volume].every(Number.isFinite))
    .filter((candle) => candleMs(candle.time) + intervalMs <= observedAt)
...
function trendScore(candles: Candle[]) {
  ...
  if (!fast || !slow || !last) return 0;
  const scale = Math.max((currentAtr ?? last * 0.005) / last, 0.0025);
  return clamp((fast / slow - 1) / (scale * 1.6), -1, 1);
}
...
function rangeWidthPct(candles: Candle[], lookback = 24) {
  ...
  if (window.length < Math.min(lookback, 12) || last <= 0) return null;
  const high = Math.max(...window.map((candle) => candle.high));
  const low = Math.min(...window.map((candle) => candle.low));
  return (high - low) / last * 100;
}

function commonMetrics(input: ShadowStrategyInput, signedScore: number, regime: MarketRegime): SignalMetric[] {
  const direction = signedScore === 0 ? 1 : Math.sign(signedScore);
  const oiScore = input.openInterestChangePct == null ? 0 : direction * clamp(input.openInterestChangePct / 4, -1, 1);
  return [
    { key: "multi-timeframe", label: "多周期结构", score: clamp(regime.trendScore, -1, 1), detail: `多周期趋势 ${(regime.trendScore * 100).toFixed(0)}`, available: input.multiTimeframeTrend != null, category: "price" },
    ...
    { key: "derivatives", label: "OI / 资金费率", score: clamp(oiScore, -1, 1), detail: `OI ${input.openInterestChangePct == null ? "--" : `${input.openInterestChangePct.toFixed(2)}%`}；资金费率 ${input.fundingRate == null ? "--" : `${(input.fundingRate * 100).toFixed(4)}%`}`, available: input.openInterestChangePct != null || input.fundingRate != null, category: "derivatives" },
  ];
}

export function classifyShadowRegime(input: ShadowStrategyInput): MarketRegime {
  const c5 = completed(input.candles5m, FIVE_MINUTES, input.observedAt);
  const c15 = completed(input.candles15m, FIFTEEN_MINUTES, input.observedAt);
  const c1h = completed(input.candles1h, ONE_HOUR, input.observedAt);
  const c4h = completed(input.candles4h, FOUR_HOURS, input.observedAt);
  const atr5 = atr(c5);
  const atrPct = atr5 != null && input.futuresPrice > 0 ? atr5 / input.futuresPrice * 100 : null;
  const compressionRatio = rollingAtrRatio(c5);
  const width = rangeWidthPct(c5);
  const t15 = trendScore(c15);
  const t1h = trendScore(c1h);
  const t4h = trendScore(c4h);
  const available = [t15, t1h, t4h].filter(Number.isFinite);
  const computedTrend = available.length ? t15 * 0.45 + t1h * 0.35 + t4h * 0.20 : 0;
  const trend = input.multiTimeframeTrend == null ? computedTrend : input.multiTimeframeTrend * 0.65 + computedTrend * 0.35;
  ...
  if (stress) {
    ...
    kind = "compression";
    reason = `短周期波动压缩至历史的 ${(compressionRatio * 100).toFixed(0)}%`;
  } else if (Math.abs(trend) >= 0.42) {
    kind = "trend";
    reason = `多周期趋势强度 ${(Math.abs(trend) * 100).toFixed(0)}`;
  } else if (Math.abs(trend) <= 0.22 && (width ?? 99) <= 5.5) {
    kind = "range";
    reason = `多周期斜率低且近端区间宽度 ${width == null ? "--" : `${width.toFixed(2)}%`}`;
  } else {
    kind = "mixed";
    reason = "趋势、震荡和波动特征暂未形成单一主导状态";
  }
  return {
    kind,
    trendScore: Number(clamp(trend, -1, 1).toFixed(4)),
...
function buildPlan(input: ShadowStrategyInput, options: {
  side: TradeSide;
  ...
  const tp1 = entry + direction * risk * (options.tp1R ?? 1);
  const tp2 = entry + direction * risk * (options.tp2R ?? 2);
  const zone = currentAtr * (options.entryZoneAtr ?? 0.18);
  const checks = [
    ...options.checks,
    { key: "risk-distance", label: "结构止损距离", passed: plannedRiskPct <= 6, required: true, detail: `${plannedRiskPct.toFixed(2)}% / 上限 6.00%` },
  ];
  return {
...
function defaultExitRules(side: TradeSide, stop: number, tp1: number, tp2: number, timeout: number): ExitRule[] {
  return [
    ...
    { code: "breakeven", label: "第一目标保护", condition: `到达 ${tp1} 后止损移动至入场价` },
    { code: "take_profit", label: "第二目标", condition: `到达 ${tp2} 完成退出` },
    { code: "structure_reversal", label: "策略结构反转", condition: "策略方向分反向达到阈值" },
    { code: "flow_reversal", label: "资金流反转", condition: "Spot CVD 与独立结构源连续反向" },
    { code: "macro_risk", label: "事件风险退出", condition: "宏观事件风险达到 85/100" },
...
function signalState(plan: EntryPlan | null, blockers: string[]): ShadowStrategyState {
  if (blockers.length) return "blocked";
  return plan?.ready ? "ready" : "watching";
}

function confidence(strength: number, quality: number, cap = 88) {
  return Math.round(clamp(50 + Math.abs(strength) * 32 + (quality - 0.65) * 24, 40, cap));
}
...
function trendPullback(input: ShadowStrategyInput, regime: MarketRegime): ShadowStrategySignal {
  ...
  const c1h = completed(input.candles1h, ONE_HOUR, input.observedAt);
  const c4h = completed(input.candles4h, FOUR_HOURS, input.observedAt);
  const closes = c5.map((candle) => candle.close);
  const fast = ema(closes.slice(-40), 9);
  const slow = ema(closes.slice(-50), 21);
  const currentAtr = atr(c5);
  const latest = c5.at(-1) ?? null;
  const previous = c5.at(-2) ?? null;
  const rsi14 = rsi(closes);
  const volRatio = volumeRatio(c5);
  const oneHour = trendScore(c1h);
  const fourHour = trendScore(c4h);
  const direction = regime.trendScore >= 0 ? 1 : -1;
  const side: TradeSide = direction > 0 ? "LONG" : "SHORT";
  const aligned = (value: number | null) => value == null ? 0 : value * direction;
  ...
  const resumed = latest != null && previous != null && fast != null && slow != null && (side === "LONG"
    ? fast > slow && latest.close > latest.open && latest.close >= fast && latest.close > previous.close
    : fast < slow && latest.close < latest.open && latest.close <= fast && latest.close < previous.close);
  const rsiHealthy = rsi14 != null && (side === "LONG" ? rsi14 >= 46 && rsi14 <= 68 : rsi14 >= 32 && rsi14 <= 54);
  const flowAligned = input.spotCvdRatio != null && aligned(input.spotCvdRatio) >= 0.01;
  const higherAligned = aligned(oneHour) >= 0.18 && aligned(fourHour) >= 0.08;
  const volumeOkay = volRatio != null && volRatio >= 0.72;
  const regimeOkay = regime.kind === "trend" || (regime.kind === "mixed" && Math.abs(regime.trendScore) >= 0.35);
  const checks: EntryCheck[] = [
    { key: "regime", label: "趋势市场", passed: regimeOkay, required: true, detail: `${regime.kind} · ${regime.reason}` },
    { key: "higher-timeframes", label: "1h/4h 同向", passed: higherAligned, required: true, detail: `1h ${(oneHour * direction).toFixed(2)} / 4h ${(fourHour * direction).toFixed(2)}` },
    { key: "pullback-location", label: "回踩位置", passed: nearFast, required: true, detail: nearFast ? "价格回到短期均线附近" : "仍离均线过远，不追价" },
    { key: "resume", label: "5m 恢复趋势", passed: resumed, required: true, detail: resumed ? "完整 5m K 线重新顺趋势收盘" : "等待完整 5m K 线恢复" },
  ...
  const strength = direction * clamp(Math.abs(regime.trendScore) * 0.45 + Math.max(0, aligned(input.spotCvdRatio)) * 1.2 + Math.max(0, aligned(oneHour)) * 0.25 + (resumed ? 0.18 : 0), 0, 1);
  const state = signalState(plan, blockers);
  return {
    strategyId: "trend_pullback",
    label: "趋势回踩",
    shadowOnly: true,
    state,
    side: blockers.length ? "WAIT" : side,
    score: Number(strength.toFixed(4)),
    confidence: confidence(strength, input.dataQuality),
    regime,
    thesis: state === "ready" ? "高周期趋势保持，5m 回踩后重新恢复，避免在趋势扩张末端追价。" : "等待高周期趋势、回踩位置与短周期恢复同时成立。",
    reasons: checks.filter((check) => check.passed).map((check) => check.label),
    blockers: [...blockers, ...checks.filter((check) => check.required && !check.passed).map((check) => `${check.label}未通过`)],
    entryPlan: plan,
    metrics: commonMetrics(input, strength, regime),
  };
}
...
function volatilityBreakout(input: ShadowStrategyInput, regime: MarketRegime): ShadowStrategySignal {
  ...
  const checks: EntryCheck[] = [
    { key: "compression", label: "突破前波动收缩", passed: compression, required: true, detail: regime.compressionRatio == null ? "ATR 压缩数据不足" : `ATR 比率 ${regime.compressionRatio.toFixed(2)}` },
    ...
    { key: "oi", label: "OI 不萎缩", passed: oiSupports, required: true, detail: input.openInterestChangePct == null ? "OI 缺失，按中性处理" : `${input.openInterestChangePct.toFixed(2)}%` },
  ...
  const strength = direction * clamp((breakout ? 0.38 : 0) + Math.max(0, (volRatio ?? 0) - 1) * 0.30 + Math.max(0, (input.spotCvdRatio ?? 0) * direction) * 1.4 + (compression ? 0.18 : 0), 0, 1);
  const state = signalState(plan, blockers);
  return {
    ...
    shadowOnly: true,
    state,
    side: blockers.length || (!breakoutLong && !breakoutShort) ? "WAIT" : side,
    score: Number(strength.toFixed(4)),
    confidence: confidence(strength, input.dataQuality),
    regime,
    thesis: state === "ready" ? "波动先收缩、再由完整 K 线放量突破，并由现货流和 OI 共同确认。" : "不追普通大阳/大阴线，只等待压缩后的有效突破。",
    reasons: checks.filter((check) => check.passed).map((check) => check.label),
    blockers: [...blockers, ...checks.filter((check) => check.required && !check.passed).map((check) => `${check.label}未通过`)],
    entryPlan: plan,
    metrics: commonMetrics(input, strength, regime),
...
function rangeReversion(input: ShadowStrategyInput, regime: MarketRegime): ShadowStrategySignal {
  ...
  const noBreakoutVolume = (volumeRatio(c5) ?? 1) <= 1.55;
  ...
  const checks: EntryCheck[] = [
    ...
    { key: "no-breakout", label: "未进入放量突破", passed: noBreakoutVolume, required: true, detail: `量能 ${(volumeRatio(c5) ?? 0).toFixed(2)}×` },
  ];
  ...
  const strength = direction * clamp(edgeDistance * 0.45 + (microstructureSupport ? 0.25 : 0) + (rsi14 == null ? 0 : Math.abs(rsi14 - 50) / 50 * 0.25), 0, 1);
  const state = signalState(plan, blockers);
  return {
    ...
    shadowOnly: true,
    state,
    side: blockers.length || !setup ? "WAIT" : side,
    score: Number(strength.toFixed(4)),
    confidence: confidence(strength, input.dataQuality, 84),
    regime,
    thesis: state === "ready" ? "只在低趋势震荡区间边缘反向交易，并要求现货流或订单簿出现吸收/背离。" : "趋势存在时自动退出候选，不用均值回归去硬扛趋势。",
    reasons: checks.filter((check) => check.passed).map((check) => check.label),
    blockers: [...blockers, ...checks.filter((check) => check.required && !check.passed).map((check) => `${check.label}未通过`)],
    entryPlan: plan,
    metrics: commonMetrics(input, strength, regime),
...
function relativeStrength(input: ShadowStrategyInput, regime: MarketRegime): ShadowStrategySignal {
  ...
  const strength = direction * clamp((relative == null ? 0 : Math.abs(relative) / 6) * 0.50 + Math.max(0, (input.multiTimeframeTrend ?? 0) * direction) * 0.28 + Math.max(0, (input.spotCvdRatio ?? 0) * direction) * 1.2, 0, 1);
  const state = signalState(plan, blockers);
  return {
    ...
    shadowOnly: true,
    state,
    side: blockers.length || !relativeStrong ? "WAIT" : side,
    score: Number(strength.toFixed(4)),
    confidence: confidence(strength, input.dataQuality, 76),
    regime,
    thesis: state === "ready" ? "只做相对 BTC/ETH 明显更强或更弱、且自身趋势与现货流同步的高流动性标的。" : "该策略证据在加密市场研究中更分歧，因此保持更低置信度上限并需要更多样本。",
    reasons: checks.filter((check) => check.passed).map((check) => check.label),
    blockers: [...blockers, ...checks.filter((check) => check.required && !check.passed).map((check) => `${check.label}未通过`)],
    entryPlan: plan,
    metrics: commonMetrics(input, strength, regime),
...
export function evaluateShadowStrategies(input: ShadowStrategyInput): ShadowStrategySignal[] {
  ...
  return [
    trendPullback(input, regime),
    volatilityBreakout(input, regime),
    rangeReversion(input, regime),
    relativeStrength(input, regime),
  ];
}
