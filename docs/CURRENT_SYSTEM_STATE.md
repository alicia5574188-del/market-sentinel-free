## 2026-09-06 — V13.1 模拟仓位下限修正待发布

新开历史路径PAPER单笔/组合预算提高为4%/12%，取消会让后续订单缩水的相关敞口折半；信号成立后按真实TP2距离反算并放大模拟名义仓位，使第二止盈扣除估计往返费用后至少30U。30U不是额外拒单条件、利润目标或上限，不改写或截短历史路径给出的市场目标；超过30U继续按真实目标计算。为表达该仓位可使用流动性、波动与强平安全边界内的杠杆空间，计划止损仍受4%约束，全部持仓计划止损合计受12%约束。现有SOL等持仓不追改原计划。本次不改五段匹配、方向、目标、退出、版本/epoch、D1或LIVE关闭，只是V13的模拟仓位政策补丁。发布证据待本次PR/merged-main workflow补入。
