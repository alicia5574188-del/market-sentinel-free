...
export function advanceForward(input:{state:ForwardState;now:number;paths:Record<string,Candle[]>;minutePaths?:Record<string,Candle[]>;daily?:Record<string,Candle[]>;
  ...
  const allowed=input.entrySymbols?new Set(input.entrySymbols):undefined,
    ...
    expectedMarkets=Math.max(1,allowed?.size??Object.keys(input.paths).length??s.selectedSymbols.length),
  ...
  if(marketReady){
    ...
    // A Worker restart loses in-memory 5m paths before the durable strategy
    // state. Keep the last confirmed whole-market map for position protection,
    // but do not manufacture new entries from a partial market.
    s.opportunities=[];
    s.extremumRegime.coverage={...(s.extremumRegime.coverage??{dailyMarkets:0,targetDailyMarkets:0,macroReady:false}),
      intradayMarkets:readyPaths,targetIntradayMarkets:expectedMarkets};
