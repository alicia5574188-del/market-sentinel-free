...
const live = (): RuntimeHealthShape => ({ state: "LIVE", stale: false, authorityReady: true, lastError: null,
  symbols: ["A", "B", "C", "D"], evidence: Object.fromEntries(["A", "B", "C", "D"].map((symbol) => [symbol, { fresh: true, ancillaryFresh: true }])) });
...
test("runtime readiness rejects every stale, incomplete, recovery, or transport-old view", () => {
  ...
  assert.equal(runtimeReady({ ...live(), symbols: ["A", "B", "C"] }), false);
  assert.equal(runtimeReady({ ...live(), symbols: ["A", "A", "C", "D"] }), false);
