...
function plan(input:Strategy2Input, side:TradeSide, stop:number, tp2R:number, mins:number, soft:EntryCheck[]):EntryPlan|null{
  ...
  return {ready:hard.every(c=>c.passed),side,entryPrice:entry,entryZone:[entry-a*.2,entry+a*.2],stopLossPrice:stop,takeProfit1Price:tp1,takeProfit2Price:tp2,riskPerUnit:risk,plannedRiskPct:riskPct,riskReward:tp2R,maxHoldingMinutes:mins,checks:[...hard,...soft.map(c=>({...c,required:false}))],exitRules:exitRules(side,stop,tp1,tp2,mins)};
}
...
function adaptLegacy(input:Strategy2Input, regime:MarketRegime, asset:Strategy2AssetRegime, s:ShadowStrategySignal):Strategy2Signal{
  ...
  const blockers=hardBlockers(input), p=s.entryPlan?{...s.entryPlan,ready:blockers.length===0,checks:s.entryPlan.checks.map(c=>({...c,required:c.key==="risk-distance"}))}:null;
  ...
  return {...s,strategyId:id,label:STRATEGY2_LABELS[id],state:blockers.length?"blocked":ready?"ready":"watching",side:blockers.length?"WAIT":side,entryPlan:p,blockers:[...blockers,...s.blockers],strategyMeta:{playbookId:PLAYBOOKS[id],assetRegime:asset,setupScore:Math.round(setup),evidenceScore:Math.round(evidence),triggerActive:trigger,hardGatePassed:blockers.length===0&&Boolean(p?.ready),candidateSide:side}};
}
...
export function evaluateStrategy2Pool(input:Strategy2Input):Strategy2Signal[]{
  ...
  const p2=make(input,regime,asset,{id:"trend_breakout",label:STRATEGY2_LABELS.trend_breakout,side:trendSide,trigger:Math.abs(input.multiTimeframeTrend??0)>=.25&&trendBreak,setup:clamp(Math.abs(input.multiTimeframeTrend??0)*70+(trendBreak?25:0)),evidence:clamp((vr-0.7)*45+signed(input.spotCvdRatio,trendSide)*500+55),stop:swingStop(rows,trendSide,a,10,.12),rr:2.1,mins:150,thesis:"趋势结构已经形成时允许突破延续，不要求每个确认指标同时完美。",soft:[{key:"trend",label:"趋势方向",passed:Math.abs(input.multiTimeframeTrend??0)>=.25,required:false,detail:""},{key:"volume",label:"量能",passed:vr>=.9,required:false,detail:`${vr.toFixed(2)}x`},{key:"flow",label:"现货流",passed:signed(input.spotCvdRatio,trendSide)>=0,required:false,detail:""}]});
  const expSide:TradeSide=move>=0?"LONG":"SHORT";
  const p5=make(input,regime,asset,{id:"expansion_momentum",label:STRATEGY2_LABELS.expansion_momentum,side:expSide,trigger:(asset==="expansion_up"||asset==="expansion_down")&&Math.abs(move)>=.2,setup:clamp(Math.abs(move)*28+(vr-1)*35+45),evidence:clamp(55+signed(input.spotCvdRatio,expSide)*450+signed(input.openInterestChangePct,expSide)*3),stop:swingStop(rows,expSide,a,8,.12),rr:2.0,mins:110,thesis:"波动已经扩张后顺着真实动量参与，但用风险倍率防止扩张末端重仓追价。",soft:[{key:"expansion",label:"波动扩张",passed:asset.startsWith("expansion"),required:false,detail:asset},{key:"flow",label:"资金流",passed:signed(input.spotCvdRatio,expSide)>=0,required:false,detail:""}]});
  ...
  const p8=make(input,regime,asset,{id:"exhaustion_reversal",label:STRATEGY2_LABELS.exhaustion_reversal,side:revSide,trigger:exhaust&&(rejection2||signed(input.spotCvdRatio,revSide)>=.004),setup:clamp((exhaust?55:0)+(rejection2?25:0)+Math.abs(input.changePercentage??0)*2),evidence:clamp(50+signed(input.spotCvdRatio,revSide)*500),stop:swingStop(rows,revSide,a,10,.22),rr:1.8,mins:100,thesis:"价格过度延伸且出现拒绝/资金流背离时测试衰竭反转。",soft:[{key:"stretch",label:"价格延伸",passed:exhaust,required:false,detail:`RSI ${r?.toFixed(1)??"--"}`},{key:"rejection",label:"拒绝确认",passed:Boolean(rejection2),required:false,detail:""}]});
  ...
  const p11=make(input,regime,asset,{id:"failed_breakout",label:STRATEGY2_LABELS.failed_breakout,side:failSide,trigger:failedHigh||failedLow,setup:clamp((failedHigh||failedLow?65:0)+(asset==="transition"?15:0)),evidence:clamp(50+signed(input.spotCvdRatio,failSide)*500+signed(input.orderBookImbalance,failSide)*90),stop:swingStop(rows,failSide,a,8,.18),rr:1.75,mins:100,thesis:"突破未延续并重新回到结构内时反向交易失败一侧。",soft:[{key:"failure",label:"假突破回收",passed:failedHigh||failedLow,required:false,detail:""}]});
  ...
  const p12=make(input,regime,asset,{id:"flow_divergence",label:STRATEGY2_LABELS.flow_divergence,side:divSide,trigger:bearish||bullish,setup:clamp((bearish||bullish?65:0)+Math.abs(input.spotCvdRatio??0)*350),evidence:clamp(55+signed(input.orderBookImbalance,divSide)*100+Math.min(20,Math.abs(input.openInterestChangePct??0)*3)),stop:swingStop(rows,divSide,a,10,.18),rr:1.8,mins:120,thesis:"价格与Spot CVD明显背离时测试价格向真实主动资金流重新收敛。",soft:[{key:"divergence",label:"价格/资金流背离",passed:bearish||bullish,required:false,detail:`价格 ${move.toFixed(2)}%`}]});
