...
export default function ForwardDashboard({data,healthy,feedAt,error,livePanel,liveEnabled}:{data:View|null;healthy:boolean;feedAt:number|null;error:string|null;livePanel:ReactNode;liveEnabled:boolean}) {
  ...
  return <main className="fr-app" data-ui-version="dark-live-v1">
    ...
    {tab==="overview"&&<>
      <section className="fr-hero"><div className="fr-hero-copy"><span className="fr-kicker">市场在变化，规则随证据更新</span><h1>{title}</h1><p>{data?.latestReason??"读取已持久化的账户、规则和观测记录；连接前不显示虚构成交或收益。"}</p><div className="fr-hero-tags"><span>前向运行 {elapsed==null?"—":fmt(elapsed,1)} 小时</span><span>{data?.policyVersion??"读取算法版本"}</span><span>新规则仅模拟</span></div></div>
