...
type Plan = Decision & { state: "PREPARED" | "TRIGGERED" | "CANCELLED"; plannedRisk: number; notional: number; expiresAt: number; leverage?: number; margin?: number; economicTarget?: number; breakoutSignalCount?: number; invalidationSignalCount?: number; invalidationSignalReason?: "TARGET_GONE_CANCEL" | "ACTIVATION_LOST_CANCEL" | "ROUTE_WEAK_CANCEL" };
...
const routeText: Record<RouteKind, string> = { LOCAL_BREAKOUT: "父区间强势突破观察", INTERNAL_ROTATION: "子区间强势迁移观察", BREAKOUT_RETEST: "突破回踩后再加速", FAILED_BREAKOUT_REVERSAL: "假突破反向反抽", EDGE_REJECTION: "旧版区间边界回撤", NODE_CONTINUATION: "高周期节点续破" };
...
function waitReason(runtime: Runtime | null, marketReady: boolean, symbol: string) {
  ...
  if (position?.status === "OPEN") {
    ...
    return "持仓由完整1分钟证据管理；软退出需收盘逆向达到0.75R，原始止损仍即时执行";
  }
  ...
  return <main>
    ...
    <section className="markets" hidden={tab !== "brain"}>{runtime?.symbols.map((symbol) => {
      ...
      return <article className="market" key={symbol}><div className="market-title"><div><small>{symbol.replace("_", "/")}</small><h2>{marketFresh ? status : "数据恢复中"}</h2></div><strong>{marketFresh ? num(evidence?.midpoint, 5) : "—"}</strong></div>
        <div className="plain-answer"><small>系统判断</small><b>{intent ? `${sideText(intent.side)} · ${intent.routeKind === "INTERNAL_ROTATION" ? "区间内部迁移" : intent.routeKind === "BREAKOUT_RETEST" ? "突破回踩延续" : intent.routeKind === "FAILED_BREAKOUT_REVERSAL" ? "假突破反转" : stateText[intent.marketState]}` : "暂时没有值得执行的方向"}</b><p>{waitReason(runtime, marketFresh, symbol)}</p></div>
        ...
        <div className="execution"><small>执行方式</small><b>{position?.status === "OPEN" ? waitReason(runtime, marketFresh, symbol) : plan?.state === "PREPARED" ? plan.routeKind === "FAILED_BREAKOUT_REVERSAL" ? (liveEnabled ? `Gate 限价等待反抽 ${num(plan.entryTrigger, 5)} · 有效至 ${time(plan.expiresAt)}` : `模拟等待反抽原边界 ${num(plan.entryTrigger, 5)} · 到期自动撤销`) : plan.routeKind === "BREAKOUT_RETEST" ? `已确认回踩守住，内部等待重新加速至 ${num(plan.entryTrigger, 5)}，0.5R外不追` : plan.marketState === "BREAKOUT" ? `内部监测 ${num(plan.entryTrigger, 5)}；仅86%确认、18%以下假突破风险和连续4次盘口允许直入` : liveEnabled ? `Gate 限价挂单等待 ${num(plan.entryTrigger, 5)} · 有效至 ${time(plan.expiresAt)}` : `模拟等待实时价格到达 ${num(plan.entryTrigger, 5)} · 软变化先观察，到期自动撤销` : intent ? "方向已形成，等待系统建立进场计划" : "继续等待完整机会"}</b></div>
    ...
    <section className="panel-list" hidden={tab !== "orders"}>{!openPositions.length && !preparedPlans.length && !recentClosedPositions.length && <div className="empty"><b>当前没有模拟订单</b><p>出现合适位置后会先显示准备计划；真实订单请进入底部「实盘」。</p></div>}
      ...
      {openPositions.map(({ symbol, position }) => <OrderCard key={symbol} symbol={symbol} side={position.side} label="持仓中" state={position.scenario} notional={position.notional} equity={runtime?.equity ?? INITIAL_EQUITY} note={waitReason(runtime, true, symbol)} values={[["进场", position.entryPrice], ["原始止损", position.initialStop], ["当前保护位", position.currentStop], ["动态目标", position.currentTarget], ["计划风险", position.plannedRisk]]} />)}
      {!!preparedPlans.length && <h2 className="order-group-title">等待进场 <span>{preparedPlans.length}</span></h2>}
      {preparedPlans.map(({ symbol, plan }) => <OrderCard key={symbol} symbol={symbol} side={plan.side} label={plan.marketState === "BREAKOUT" ? "等待确认" : "等待触发"} state={plan.marketState} notional={plan.notional} equity={runtime?.equity ?? INITIAL_EQUITY} leverage={plan.leverage} margin={plan.margin} note={(plan.invalidationSignalCount ?? 0) > 0 ? `软失效观察 ${plan.invalidationSignalCount}/2，计划仍锁定；连续确认后才撤销。有效至 ${time(plan.expiresAt)}。` : plan.routeKind === "BREAKOUT_RETEST" ? `完整1分钟回踩原边界并守住；等待重新越过回踩K线极值后IOC，不追0.5R外价格。有效至 ${time(plan.expiresAt)}。` : plan.routeKind === "FAILED_BREAKOUT_REVERSAL" ? `完整1分钟扫边并收回；等待区间内部反抽原边界后反向限价成交。有效至 ${time(plan.expiresAt)}。` : plan.marketState === "BREAKOUT" ? `${plan.routeKind === "INTERNAL_ROTATION" ? "子区间迁移" : "父区间突破"}内部观察；仅A级强势允许直入，连续盘口确认 ${(plan.breakoutSignalCount ?? 0)}/4。有效至 ${time(plan.expiresAt)}。` : `本挂单有效至 ${time(plan.expiresAt)}；短周期变化先观察，硬失效或到期才立即撤销。`} values={[[plan.marketState === "BREAKOUT" ? (plan.routeKind === "BREAKOUT_RETEST" ? "回踩再加速位" : plan.routeKind === "INTERNAL_ROTATION" ? "子区间观察位" : "父区间观察位") : plan.routeKind === "FAILED_BREAKOUT_REVERSAL" ? "反抽进场" : "触发进场", plan.entryTrigger], ["结构止损", plan.invalidation], ["实际第一目标", plan.target], ["计划风险", plan.plannedRisk]]} />)}
