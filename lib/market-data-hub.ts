 ...
 * Bybit, OKX and MEXC are primary independent public analysis feeds; Bitget and Binance are best-effort WAF-backed-off fallbacks. Symbols are mapped
 * only by exact USDT contract name (FOO_USDT <-> FOOUSDT); no heuristic aliasing.
 */
export type MarketSource="BYBIT"|"OKX"|"MEXC"|"BITGET"|"BINANCE";
...
export class MarketDataHub{
  ...
  async refresh(now=Date.now()){
    ...
    const [bybit,okx,mexc,bitget,binance]=await Promise.allSettled([
      this.fetchBybit(now),this.fetchOkx(now),this.fetchMexc(now),
    ...
    if(mexc.status==="fulfilled"){this.mexc=mexc.value;this.ok("MEXC",now,mexc.value.size);}
    else this.fail("MEXC",now,mexc.reason);
    ...
    if(bybit.status==="rejected"&&okx.status==="rejected"&&mexc.status==="rejected"
      &&(bitget.status==="rejected"||bitget.value===null)&&(binance.status==="rejected"||binance.value===null))
      throw new Error("Bybit/OKX/MEXC/Bitget/Binance public market data unavailable");
  }
  ...
  private fail(source:MarketSource,now:number,error:unknown){const prior=this.health[source],message=error instanceof Error?error.message.slice(0,160):"unknown",
    failures=Math.min(99,prior.failures+1),isWaf=(source==="BITGET"||source==="BINANCE")&&/market source 403/.test(message),
  ...
  private async fetchMexc(now:number){
    type Row={symbol?:string;lastPrice?:number|string;bid1?:number|string;ask1?:number|string;amount24?:number|string;
      riseFallRate?:number|string;timestamp?:number|string};
    type Res={success?:boolean;code?:number;data?:Row|Row[]};
    const body=await json<Res>(`${MEXC}/api/v1/contract/ticker`,BULK_TIMEOUT_MS);
    if(body.success!==true||body.code!==0||!body.data)throw new Error("MEXC ticker payload");
    const list=Array.isArray(body.data)?body.data:[body.data],out=new Map<string,HubQuote>();
    for(const row of list){const symbol=(row.symbol??"").endsWith("_USDT")?row.symbol!:null;if(!symbol)continue;
      ...
      out.set(symbol,{source:"MEXC",symbol,observedAt,last,bid,ask,volume24hUsd:Math.max(0,Number(row.amount24??0)),
        change24hRate:Number(row.riseFallRate??0)});
    }
  ...
  quote(symbol:string,now=Date.now()):ConsensusQuote|null{
    const rows=[this.bybit.get(symbol),this.okx.get(symbol),this.mexc.get(symbol),this.bitget.get(symbol),this.binance.get(symbol)]
    ...
    const directional=rows.find(q=>q.source==="BYBIT")??rows.find(q=>q.source==="OKX")??rows.find(q=>q.source==="MEXC")??rows.find(q=>q.source==="BITGET");
  ...
  supports(symbol:string){return this.bybit.has(symbol)||this.okx.has(symbol)||this.mexc.has(symbol)||this.bitget.has(symbol)||this.binance.has(symbol);}
  ...
  async candles(symbol:string,interval:"1m"|"5m",limit=120):Promise<{source:MarketSource;rows:HubCandle[]}|null>{
    ...
    const all:MarketSource[]=["BYBIT","OKX","MEXC","BITGET","BINANCE"];
    const order:MarketSource[]=preferred?[preferred,...all.filter(source=>source!==preferred)]:all;
    for(const source of order){
      if((source==="BITGET"||source==="BINANCE")&&Date.now()<this.health[source].nextRetryAt)continue;
      try{const rows=source==="BYBIT"?await this.bybitCandles(external,interval,limit)
        ...
        :source==="MEXC"?await this.mexcCandles(symbol,interval,limit)
  ...
  private async mexcCandles(symbol:string,interval:"1m"|"5m",limit:number){
    type Data={time?:number[];open?:Array<number|string>;close?:Array<number|string>;high?:Array<number|string>;low?:Array<number|string>;vol?:Array<number|string>};
    type Res={success?:boolean;code?:number;data?:Data};
    const n=Math.max(2,Math.min(500,Math.floor(limit))),seconds=interval==="1m"?60:300,granularity=interval==="1m"?"Min1":"Min5",
  ...
  status(now=Date.now()){
    const sources=(["BYBIT","OKX","MEXC","BITGET","BINANCE"] as MarketSource[]).map(source=>({source,...this.health[source],
