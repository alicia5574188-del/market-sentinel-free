...
function waitReason(runtime: Runtime | null, marketReady: boolean, symbol: string) {
  ...
  if (plan?.state === "PREPARED") return (plan.invalidationSignalCount ?? 0) > 0
    ...
    : plan.marketState === "BREAKOUT"
      ? `距离触发价约 ${num(distancePct(evidence.midpoint, plan.entryTrigger), 2)}%；触及后还需完整1分钟收在突破位外`
  ...
  return <main>
    ...
    <section className="markets" hidden={tab !== "brain"}>{runtime?.symbols.map((symbol) => {
      ...
      return <article className="market" key={symbol}><div className="market-title"><div><small>{symbol.replace("_", "/")}</small><h2>{marketFresh ? status : "数据恢复中"}</h2></div><strong>{marketFresh ? num(evidence?.midpoint, 5) : "—"}</strong></div>
        ...
        <div className="execution"><small>执行方式</small><b>{position?.status === "OPEN" ? waitReason(runtime, marketFresh, symbol) : plan?.state === "PREPARED" ? liveEnabled ? `${plan.marketState === "BREAKOUT" ? "Gate 价格触发" : "Gate 限价"}挂单等待 ${num(plan.entryTrigger, 5)} · 有效至 ${time(plan.expiresAt)}` : `模拟等待实时价格到达 ${num(plan.entryTrigger, 5)} · 软变化先观察，到期自动撤销` : intent ? "方向已形成，等待系统建立进场计划" : "继续等待完整机会"}</b></div>
