## 2026-09-06 — V13.2 持续PAPER与五段叠加线图

- 历史路径PAPER取消连续亏损和当日模拟亏损暂停，持续开仓以积累诊断样本；硬资金、4%/12%组合、报价、D1及LIVE关闭边界不变。
- 顶部改为当前真实收盘线与最相似5段真实历史收盘线的一张叠加图，参考点归零但数据不平均、不预测、不虚构。

## 2026-09-06 — V13.1 每单TP2净利30U与模拟仓位放大完成

- 历史路径PAPER新订单的单笔/组合风险从1%/3%提高至4%/12%，取消会让后续订单缩水的相关敞口折半，所有订单统一受12%组合计划止损约束；三连亏暂停保留，日亏闩锁与组合预算统一为12%。
- 信号成立后由唯一开仓仓位器按真实TP2距离反算并放大名义仓位，使TP2扣费后至少30U；30U不是额外拒单条件、目标或上限，不伸展或截短市场目标，超过则完整保留。
- 现有订单经济计划不可变，不做中途放大；五段匹配、方向、0.8R路径边界、2%保护、版本/epoch、D1和LIVE关闭均不变。
- PR #160完成倒算放大，PR #161消除后续相关订单折半；最终main `7a7a276` / workflow `33988357115` / Worker `62a2c25b-fe04-4d1b-b0df-ba5e7d5a825a` 的验证、部署和运维验收全部通过。
