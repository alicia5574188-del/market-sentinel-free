当前最高优先修正：历史路径PAPER不再因连续亏损或当日模拟亏损暂停新开仓，持续运行以暴露问题；保留资金/保证金、单笔4%/组合12%、报价、重复计划、D1及LIVE关闭等硬边界。顶部不用蜡烛图，把当前真实收盘走势与决策使用的最相似5段真实历史收盘走势叠加到一张线图；各自在参考点归零，历史线右侧仍是当时实际发生的后一小时。只走一次PR、CI与部署。
