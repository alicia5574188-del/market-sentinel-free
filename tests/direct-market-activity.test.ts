...
function candidate(observedAt: number): DirectMarketCandidate {
  return { symbol: "BTC_USDT", batchId: "batch", observedAt, freshness: "FRESH", scanStage: "DEEP",
    ...
    netEdgeR: 0.2, confidence: 65, setup: "HISTORICAL_ANALOG", setupLabel: "历史相似预测", setupScore: 65,
    setupEvaluations: [{ setup: "HISTORICAL_ANALOG", setupLabel: "历史相似预测", side: "SHORT", score: 65, triggered: true, qualified: true, selected: true, blockers: [] }],
    decision: "SHORT", entryZone: [99,100], invalidationPrice: 101, targets: [98,97], evidence: [], counterEvidence: [],
    checks: [{ key: "setup", label: "历史相似预测", passed: true, detail: "" }], candles5m: [], assetRegime: "transition", maxHoldingMinutes: 60 };
}
test("one active prediction has separate signal, qualification, rejection and actual-open counts", () => {
  ...
  const second = recordDirectTwelveHourActivity({ activity: first, candidate: candidate(now + INTERVAL_MS), openedSetup: "HISTORICAL_ANALOG", openReason: "已开仓", expectedIntervalMs: INTERVAL_MS });
...
test("old strategy activity cannot crash or contaminate the new forecast epoch", () => {
  ...
  assert.equal(next.current.evaluations, 1); assert.equal(next.current.setups[0].setup, "HISTORICAL_ANALOG");
...
test("a forecast with insufficient evidence cannot count as a qualified entry or an entry rejection", () => {
  const c = candidate(Date.UTC(2026,8,5,12)); c.decision = "WAIT";
  c.setupEvaluations![0] = { ...c.setupEvaluations![0], triggered: false, qualified: false, blockers: ["独立相似片段不足"] };
  ...
  assert.equal(activity.current.setups[0].leadingBlocker, "独立相似片段不足");
});
