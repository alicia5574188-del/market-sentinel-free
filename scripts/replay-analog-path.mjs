...
for(const symbol of coins){
 try{
  ...
  const rows=cleanAnalogCandles(payload.map(c=>Array.isArray(c)?{time:+c[0],volume:+c[1],close:+c[2],high:+c[3],low:+c[4],open:+c[5]}:{time:+c.t,volume:+c.v,close:+c.c,high:+c.h,low:+c.l,open:+c.o}),Date.now());
  if(rows.length<200)throw new Error('完整历史不足200根');if(symbol==='BTC_USDT')btc=rows;
  const ticker=tickers.find(t=>t.contract===symbol),volumeUsd=Number(ticker?.volume_24h_usd??ticker?.volume_24h_quote??0);
  let position=null,intent=undefined,lastExit=0,signals=0,closed=0,wins=0,netR=0;const blockers={};
  ...
  reports.push({symbol,available:true,bars:rows.length,from:rows[start].time*1000,to:rows.at(-1).time*1000+300000,signals,closed,wins,netR:+netR.toFixed(3),stillOpen:!!position,blockers});
