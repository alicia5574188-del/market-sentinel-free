...
export function forecastCausalPath(feature:PredictiveFeatureVector,quote?:PredictiveQuote):PredictivePathForecast{
  const g=accessor(feature),e=evidence(feature),
    ...
    atr=Math.max(g("atr14"),.001),atrSlow=Math.max(g("atr28"),.001),
    normalAtr=Math.max(.001,Math.min(atr,atrSlow*1.15)),
    targetBarrier=clamp(Math.max(.008,normalAtr*1.15),.008,.02),
    riskBarrier=clamp(Math.max(.0045,normalAtr*.65),.0045,.012),
    ...
    chaseUp=Math.max(0,g("ret_3")-normalAtr*1.8),chaseDown=Math.max(0,-g("ret_3")-normalAtr*1.8),
    longRegret=Math.max(0,(Math.max(0,overextension)*.52+Math.max(0,-shortPressure)*.20)*normalAtr+chaseUp*.60),
    shortRegret=Math.max(0,(Math.max(0,-overextension)*.52+Math.max(0,shortPressure)*.20)*normalAtr+chaseDown*.60),
    ...
    regretMax=clamp(normalAtr*.38,.0012,.0048),
