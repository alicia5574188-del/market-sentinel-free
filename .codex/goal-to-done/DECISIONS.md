...
- `脉折` and `缓折` are separate medium/slow completed-path phase mechanisms. They compare earlier path displacement with the latest six-segment contribution, require a completed opposite reclaim, then use synchronized market breadth to choose either isolated reversal or crowded-direction continuation. They are normalized by current path range and therefore do not require a fixed absolute volatility threshold.
