...
import {evaluateMultiTurnEntryMemory,MULTI_TURN_MIN_REENTRY_MS,MULTI_TURN_SAME_SIDE_LOSS_COOLDOWN_MS,
...
test("every symbol lifecycle has a ten-minute minimum separation",()=>{
  const d=evaluateMultiTurnEntryMemory({now:NOW,symbol:"AKE_USDT",side:"SHORT",recent:[row(2*60_000,2)]});
...
test("three recent losses create only a temporary one-hour cluster cooldown",()=>{
  const recent=[row(15*60_000,-4),row(40*60_000,-3,"SHORT"),row(70*60_000,-2),row(90*60_000,1)];
