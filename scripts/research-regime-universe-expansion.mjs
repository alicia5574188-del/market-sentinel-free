...
import { evaluateRegimePortfolio, initialRegimePortfolio, REGIME_UNIVERSE } from "../lib/regime-portfolio.ts";
...
function replay(symbols){let state=initialRegimePortfolio(raw.from*1000);const hourlyPaths=Object.fromEntries(symbols.map(s=>[s,[]]));const symbolSet=new Set(symbols);let peak=5000,maxDd=0;let lastHour=null;const equityPath=[];
 for(const time of allTimes){const quotes={};for(const s of symbols){const r=fiveIndex.get(s)?.get(time);if(!r)continue;quotes[s]={bestBid:r.open,bestAsk:r.open,observedAt:time*1000,fresh:true,completedMinuteAt:(time-300)*1000};}
   ...
   const hour=Math.floor((time-1)/3600)*3600;if(hour!==lastHour){lastHour=hour;for(const s of symbols){const h=hourIndex.get(s)?.get(hour);if(h){hourlyPaths[s].push(h);if(hourlyPaths[s].length>721)hourlyPaths[s].shift();}}state=evaluateRegimePortfolio({state,hourly:hourlyPaths,quotes,contracts,now:time*1000});}
   else state=evaluateRegimePortfolio({state,hourly:hourlyPaths,quotes,contracts,now:time*1000});
   const eq=sum(Object.values(state.accounts).map(a=>a.equity));peak=Math.max(peak,eq);maxDd=Math.max(maxDd,(peak-eq)/peak);equityPath.push([time,eq]);
 }
...
const evalMonths=raw.months.slice(6);const period=(result,months)=>{const pnl=sum(months.map(m=>result.monthly[m]??0));return{months,trades:null,netPnl:pnl,positiveMonths:months.filter(m=>(result.monthly[m]??0)>0).length};};
const satellite=EXPANDED.filter(s=>!CORE.includes(s));const satellitePnl=sum(satellite.map(s=>expanded.bySymbol[s]??0));const freqGain=expanded.trades/Math.max(core.trades,1)-1;const gates={allCorePresent:CORE.every(s=>EXPANDED.includes(s)),hasSatellites:satellite.length>=2,totalPositive:expanded.netPnl>0,profitFactor:expanded.profitFactor>=1.10,maxDrawdown:expanded.maxDrawdown<=0.12,positiveMonthMajority:expanded.positiveMonths>=Math.ceil(expanded.activeMonths*.55),frequencyGain25pct:freqGain>=.25,satellitesNonNegative:satellitePnl>=0,doesNotLoseMoreThanCore:expanded.netPnl>=core.netPnl*.85};
