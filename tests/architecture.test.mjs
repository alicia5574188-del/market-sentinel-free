...
test("Forward Relation analysis remains multi-source while Gate stays execution-only",async()=>{
  ...
  assert.match(hub,/class MarketDataHub/);assert.match(hub,/BYBIT/);assert.match(hub,/OKX/);assert.match(hub,/MEXC/);assert.match(hub,/BITGET/);assert.match(hub,/BINANCE/);
  assert.match(worker,/private marketHub = new MarketDataHub/);
  assert.match(worker,/Gate public websocket is execution-only/);
  assert.match(worker,/Bybit\/OKX\/MEXC remain the normal scan surface/);
