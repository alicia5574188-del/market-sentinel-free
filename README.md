# LIVE数据连续性与大跌进场修复 — 2026-09-23

Gate私有签名请求现在使用Cloudflare支持的手动重定向模式，并在本地拒绝全部3xx；
读取仍可在两个官方合约域名间做完整响应对冲，写请求绝不重放。30标的扫描优先
保留持仓、信号和进行中的RegionLaunch，固定观察BTC/ETH/SOL，并分配有限的高流动性
连续性与轮换探索槽；这些选择只保证行情路径不断，不直接决定多空或绕过进场条件。

较慢但有效的5分钟区间外收盘，除原“小回调后重启”外，可由首根连续、完整且形态
严格的1分钟K继续突破5分钟极值确认；微小漂移、长影线、缺口、回区间和过度追价仍
不能进场。REJECTION回归单仅在至少8个市场就绪且5m/15m方向和广度极端同步反向时
暂缓，顺势AnchorFlow/RegionLaunch不受影响。账户、持仓、历史、风险、成本、止损、
实盘开关和凭据不重置。具体边界见`research/LIVE_AND_SHOCK_CONTINUITY_2026-09-23.md`。
...
当前持久化账户格式：`forward-relations-v1.0`；策略权威：`forward-path-relation-v3`。**多源真实行情 + 独立 1,000 USDT PAPER 决策源 + Gate LIVE/会员 LIVE 同源跟随**。这是功能上线，不代表已经证明盈利或月复利翻倍；LIVE 不自行重算策略，只执行持久化 PAPER 生命周期。

## 当前运行方式

- 使用现有多源行情层扫描最多30个Gate可交易标的；每个完成5分钟帧可生成一个根样本。同一根样本从15分钟起逐步成熟，并持续补全5/10/15/20/30/45/60分钟真实路径；检查点不是独立样本，不重复投票。升级会迁移已有Forward反应样本，不重置账户或已学证据。
- 根样本允许每5分钟连续产生，但统计资格按非重叠时间组聚合，相关币种先在同一市场事件内平均，避免重叠根样本和全市场共振被当成大量独立投票；路径断口作废。市场测量不是影子订单，不设连胜晋级。
- 规则仍保持至多两个特征条件，但15/30/45/60分钟只用于学习同一条件族的最佳持仓，而不是制造四个独立策略。训练/检查按非重叠时间组验证；规则同时产出方向、正常MAE、正反馈期限、最佳持仓、最大持仓、目标空间和利润保留率。结构止损与账户风险上限仍是硬边界。
