...
- Position books and owner-controlled LIVE reconciliation run before the optional bulk scan. A failed bulk scan preserves the last good radar snapshot, retries with bounded 15/30/60-second backoff, never becomes a global execution fault, and cannot authorize new strategy observations once that snapshot is older than 30 seconds.
