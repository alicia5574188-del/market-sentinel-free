...
- A detected 15m balance exposes upper/lower breakouts and edge rejections as soft routes. While that local route map exists, it is the only entry authority: the legacy all-timeframe fallback cannot create a distant order. A farther 1h/4h node is split behind a 0.65%–0.9% adaptive 15m measured objective; the system exits there unless fresh continuation strength promotes the next node. Edge rejection first targets 70% of the range rather than assuming the complete opposite edge will be reached.
