...
function buildExitProfiles(rows: ParsedArenaRow[]): Strategy2ArenaExitProfile[] {
...
function buildPlaybooks(rows: ParsedArenaRow[]): Strategy2ArenaPlaybook[] {
  ...
  return [...groups.entries()].map(([playbook, bucket]) => {
    ...
    const recentExpectancyR = average(latest(bucket, 20).map((row) => row.resultR));
    ...
    const rank = (state: Strategy2ArenaPlaybook["state"]) => state === "negative" ? 4 : state === "positive" ? 3 : state === "watch" ? 2 : 1;
    return rank(b.state) - rank(a.state) || b.sampleCount - a.sampleCount;
  });
}

function buildHeatmap(rows: ParsedArenaRow[]): Strategy2ArenaCell[] {
  const groups = new Map<string, ParsedArenaRow[]>();
...
export function buildStrategy2LearningArena(rows: Strategy2ArenaTradeRow[], generatedAt = Date.now()): Strategy2LearningArena {
  ...
  const periodDeltaR = forward.expectancyR != null && preForward.expectancyR != null ? forward.expectancyR - preForward.expectancyR : null;
  ...
  const defensive = (stats.sampleCount >= 40 && (stats.averageNetPct ?? 0) < -0.02 && (stats.profitFactor ?? 0) < 0.95)
    || (stats.recentSampleCount >= 20 && (stats.recentAverageNetPct ?? 0) < 0 && (stats.recentProfitFactor ?? 0) < 1);
  ...
  return {
    ...
    forwardEvidence: {
      ...
      periodDeltaR,
      interpretation: "period_shift_only",
      note: "这是学习启用前后两个时期的变化，不是同一市场的因果对照，因此不等于 Learning Alpha。",
    },
    learningProof: {
      status: "COLLECTING",
      learningAlphaR: null,
    ...
    exits: buildExitProfiles(parsed),
    playbooks: buildPlaybooks(parsed),
    heatmap: buildHeatmap(parsed),
    safety: {
      changesTradingLogic: false,
      changesRisk: false,
      changesExecution: false,
