...
- Actual chart candles are mirrored from the same authority to D1 at most once every five minutes; one continuously open operator page adds only cached D1 reads and no Durable Object request load. Each PAPER fill also keeps a bounded entry/exit review window in `paper_events`, so History can draw the real Gate 1m candles and exact entry/exit markers without replaying old market data.
...
Stops and exits are structural and dynamic. A stop tightens only in 0.1R steps and never widens. Structural stops execute immediately; changing liquidity targets or opposite utility must persist across two distinct completed 1m candles before they can close a position. Every entry must still have at least 1.2:1 net reward/risk after modeled 0.18% round-trip friction, while REVERSAL/RANGE must reconfirm absorption at the frozen trigger. Aggregate open structural loss is hard-capped at 5% of the applicable PAPER or actual Gate equity, including fees and stress slippage; LIVE recalculates notional, margin, leverage, and risk from the actual fill. There is no PnL pause, fixed take-profit, or fixed holding time. The Durable Object owns both execution lanes; D1 is only an idempotent PAPER/review mirror and encrypted credential store.
