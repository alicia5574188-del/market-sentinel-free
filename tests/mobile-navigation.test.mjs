...
test("Resonance mobile navigation is one fixed three-destination control", async () => {
  const [page, css] = await Promise.all([
    readFile(new URL("../app/page.tsx", import.meta.url), "utf8"),
    readFile(new URL("../app/resonance.css", import.meta.url), "utf8"),
  ]);
  assert.match(page, /const NAV: Tab\[\] = \["大脑", "订单", "管理"\]/);
  const navSource = page.match(/const NAV: Tab\[\][^\n]+/)?.[0] ?? "";
  assert.doesNotMatch(navSource, /机会|雷达|实盘|设置/);
  assert.match(page, /<nav className="rz-nav"/);
  const navBlock = css.match(/\.rz-nav \{([^}]*)\}/s)?.[1] ?? "";
  assert.match(navBlock, /position: fixed/);
  assert.match(navBlock, /grid-template-columns: repeat\(3,minmax\(0,1fr\)\)/);
...
test("dashboard has one main poll and live polling only on management tab", async () => {
  const page = await readFile(new URL("../app/page.tsx", import.meta.url), "utf8");
  ...
  assert.match(page, /if \(tab !== "管理"\) return/);
