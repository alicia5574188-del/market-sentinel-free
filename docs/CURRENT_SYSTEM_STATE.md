## 2026-09-06 — V13.2 持续运行与叠加线图

历史路径PAPER不因连续亏损或当日模拟亏损暂停，继续运行以积累可诊断订单；硬资金、4%/12%组合、报价、D1和LIVE关闭边界保留。顶部用一张归零叠加线图显示当前真实收盘走势与最相似5段真实历史收盘走势，历史参考点后仍为当时实际后一小时。

## 2026-09-06 — V13.1 每单30U模拟仓位已上线

新开历史路径PAPER单笔/组合预算提高为4%/12%，取消会让后续订单缩水的相关敞口折半；信号成立后按真实TP2距离反算并放大模拟名义仓位，使第二止盈扣除估计往返费用后至少30U。30U不是额外拒单条件、利润目标或上限，不改写或截短历史路径给出的市场目标；超过30U继续按真实目标计算。为表达该仓位可使用流动性、波动与强平安全边界内的杠杆空间，计划止损仍受4%约束，全部持仓计划止损合计受12%约束。现有持仓不追改原计划。本次不改五段匹配、方向、目标、退出、版本/epoch、D1或LIVE关闭。PR #160/#161已上线，最终main `7a7a276`、workflow `33988357115`、Worker `62a2c25b-fe04-4d1b-b0df-ba5e7d5a825a` 验收通过。

## 2026-09-06 — 当前生产状态：V13运行中

PR #158 已合并为main `86578d2`，workflow `33984709869` verify/deploy/operations全绿，Worker `c69ee06f-08f1-4bfc-9f63-0d93870a419f`。0031 completed，active brain为`direct-market-brain-v13-five-analog-candles`、target为空、legacy open PAPER为0。V13固定最相似5段并传输/展示每段真实24+12根OHLC；历史路径PAPER风险1%/组合3%，经济边界0.8R，其他安全边界及LIVE关闭保留。真实12小时回放42信号、21完成、7胜、+2.891R，另2笔未完成；这是有限PAPER证据，不是盈利保证。

线上浏览器确认当前24根K线及5段各36根真实历史K线均可见，旧归一化灰/金折线已移除；页面显示单笔风险1.00%。验收时SOL 60%先上，计划等待冻结回撤价103.7071，当前空仓属于等待入场价而不是旧1.2R拦截。scanner/position均live、6/6币分析、无错误；UTC日D1 929963读/2145写，向外取整24h 1188918读/2880写，低于3250000/65000安全线。
...
线上页面当前显示运行中，V12新轮次01:48:57开始。当前SOL有64%先下方向，但因2%保护/1.2倍扣费后完整风险空间不满足而诚实等待；这不是旧历史期望闸门。PR真实12小时回放为11信号、9完成、+0.963R，证明订单路径可达，但不能承诺当前时刻立即成交或未来收益。Gate实盘新开仓仍关闭。
