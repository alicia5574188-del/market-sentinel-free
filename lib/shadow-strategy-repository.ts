...
function traderIdForRow(row: ClosedRow) {
  return HUMAN_TRADERS.find((trader) => row.regime?.includes(trader.tag))?.id ?? null;
}
...
async function loadExecutionGovernor(): Promise<ExecutionGovernor> {
  ...
  const stats = calculateStrategyStatistics(rows);
  const lossStreak = currentLossStreak(rows);
  const streakTraderIds = new Set(rows.slice(0, lossStreak).map(traderIdForRow).filter(Boolean));
  const allTraderIds = new Set(rows.map(traderIdForRow).filter(Boolean));
  const diversifiedLossStreak = streakTraderIds.size >= 2 ? lossStreak : 0;
  const hasCrossTraderEvidence = allTraderIds.size >= 2;
  const recentWeak = hasCrossTraderEvidence && stats.recentSampleCount >= 10
    && (stats.recentAverageNetPct ?? 0) < 0
    && (stats.recentProfitFactor ?? 0) < 0.9;
  const stronglyWeak = hasCrossTraderEvidence && stats.recentSampleCount >= 16
    && (stats.recentAverageNetPct ?? 0) < -0.08
    && (stats.recentProfitFactor ?? 0) < 0.75;
  const structurallyBroken = hasCrossTraderEvidence && stats.sampleCount >= 30
    && (stats.averageNetPct ?? 0) < -0.12
    && (stats.profitFactor ?? 0) < 0.65;

  // Account-level protection must represent account-wide damage. A losing run
  // generated by only one trader belongs to that trader's circuit breaker and
  // must not silently raise Dennis/Raschke/Turtle Soup peers into stricter gates.
  const state: ExecutionGovernor["state"] = diversifiedLossStreak >= 8 || structurallyBroken
    ? "PAUSED"
    : diversifiedLossStreak >= 6 || stronglyWeak
      ? "DEFENSIVE"
      : diversifiedLossStreak >= 4 || recentWeak
        ? "CAUTION"
        : "NORMAL";
  ...
  const concentratedLossNote = lossStreak > 0 && streakTraderIds.size < 2
    ? ` 最近 ${lossStreak} 笔亏损集中于单一交易员，只由该交易员独立熔断，不升级全局门槛。`
    : lossStreak > 0
      ? ` 最近连续亏损涉及 ${streakTraderIds.size} 位交易员。`
      : "";
  const reason = state === "PAUSED"
    ? `Human Risk Governor 全局暂停新开仓：跨交易员连续亏损 ${diversifiedLossStreak} 笔，n=${stats.sampleCount}，PF ${stats.profitFactor?.toFixed(2) ?? "--"}。已有仓位继续保护。`
    : state === "DEFENSIVE"
      ? `Human Risk Governor 全局防守：跨交易员连续亏损 ${diversifiedLossStreak} 笔；只允许已验证且高确信 Setup。${cooling.length ? ` 独立冷却：${cooling.join("、")}。` : ""}${concentratedLossNote}`
      : state === "CAUTION"
        ? `Human Risk Governor 全局谨慎：跨交易员连续亏损 ${diversifiedLossStreak} 笔；不靠放宽门槛增加频率。${cooling.length ? ` 独立冷却：${cooling.join("、")}。` : ""}${concentratedLossNote}`
        : `Human Risk Governor 正常：n=${stats.sampleCount}，原始连续亏损 ${lossStreak}，最近 PF ${stats.recentProfitFactor?.toFixed(2) ?? "--"}。${cooling.length ? ` 独立冷却：${cooling.join("、")}。` : ""}${concentratedLossNote}`;
