...
The complete local verification suite passed. Production rendered 216 actual candles across the three market cards, but repeated ad-hoc Gate requests from the public route proved unreliable even with a longer timeout. The final design therefore serves the same completed candles already collected by the continuously running MarketStream authority, with a short public cache and bounded warmup retries. Period identity is tracked separately so an old 15m chart can never be mislabeled as 1h. Acceptance still requires the final deployment, advancing production health, candle smoke test, and live page inspection.
