...
function breakoutCandles():Candle[]{const rows=candles();const priorHigh=Math.max(...rows.slice(-25,-1).map(c=>c.high));const last=rows.at(-1)!;const close=priorHigh+.18;rows[rows.length-1]={...last,open:priorHigh+.04,high:close+.001,low:priorHigh-.04,close,volume:1700};return rows}
...
test("trend breakout does not become ready when spot flow points against the proposed direction",()=>{const rows=breakoutCandles();const signals=evaluateStrategy2Pool(input({spotCvdRatio:-.06,candles5m:rows,futuresPrice:rows.at(-1)!.close}));const p2=signals.find(s=>s.strategyId==="trend_breakout");assert.ok(p2);assert.equal(p2!.strategyMeta.triggerActive,true);assert.equal(p2!.state,"watching");const flow=p2!.entryPlan?.checks.find(c=>c.key==="flow");assert.ok(flow);assert.equal(flow!.required,true);assert.equal(flow!.passed,false);assert.ok(p2!.blockers.some(reason=>reason.includes("现货流方向确认不足")))});
