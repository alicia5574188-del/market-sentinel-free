...
- Gate 实盘资金风控继续按权益比例计算：单笔结构止损风险 ≤ 当前权益 1%，单笔保证金 ≤ 当前权益 20%，保守 TP2 预计净利润 ≥ 当前权益 1.5%；Gate 当日已实现亏损达到当日参考权益 3% 或权益较实盘峰值回撤达到 10% 后锁定新开仓。最多同时 3 个实盘仓位，但同一方向最多 2 个，避免三个高度相关的 LONG 或 SHORT 同时叠满止损风险。真实成交后会按成交价和当前权益再次复核。
- 新 HTE 3.1 模拟订单使用独立、可见的仓位经济模型：结构止损风险目标约为模拟权益 4%，允许范围为 3%–5%；扣除往返成本后的 TP2 净利润目标为权益 5%–20%。模拟杠杆会根据流动性、波动率、数据质量和爆仓缓冲自适应选择，最高 50x；这套模拟仓位公式不会静默改变 Gate 实盘风险。
- Gate 实盘 TP2 平仓使用收益闸门内的受限滑点；保护止损和紧急平仓优先可靠退出，使用 Gate 合约默认市价滑点上限。极端跳空时实际止损仍可能超过计划的 1% 风险。
