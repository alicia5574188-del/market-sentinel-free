...
import { evaluateHte31PerformanceCell } from "./hte31-performance-gate";

export type Hte31OptimizationTrade = Pick<typeof hte31Trades.$inferSelect,
  "id" | "symbol" | "traderId" | "setupId" | "side" | "assetRegime" | "entryPrice" | "initialStopPrice"
  | "riskBudgetUsdt" | "netPnlUsdt" | "exitCode" | "mfePct" | "maePct" | "holdMinutes" | "target1HitAt"
  | "stopRecovery" | "postExitLabel" | "exitEfficiency" | "exitAt">;
...
export function buildHte31OptimizationAnalysis(rows: Hte31OptimizationTrade[]) {
  ...
  if (overall.samples < 12) {
    findings.push({
      priority: "observe",
      code: "sample_size",
      evidence: `当前只有 ${overall.samples} 笔已平仓样本`,
      action: "继续模拟和 Shadow 采样；不因为小样本全局提高入场门槛。",
    });
  }
  if (overall.samples >= 6 && overall.expectancyR < 0) {
    findings.push({
      priority: "high",
      code: "negative_expectancy",
      evidence: `整体 Exp ${overall.expectancyR.toFixed(2)}R，PF ${overall.profitFactor == null ? "--" : overall.profitFactor.toFixed(2)}`,
      action: "保持 Gate 新开仓负期望门控；模拟继续运行并优先修复拖累最大的组合。",
    });
  }
  if (overall.samples >= 6 && overall.payoffRatio != null && overall.payoffRatio < 1.2) {
    findings.push({
      priority: "high",
      code: "payoff_structure",
      evidence: `平均盈利 ${overall.averageWinR?.toFixed(2)}R，平均亏损 ${overall.averageLossR?.toFixed(2)}R，Payoff ${overall.payoffRatio.toFixed(2)}`,
      action: "优先检查止盈兑现、TP1 后回本止损和 timeout，而不是继续收紧入场过滤。",
    });
  }
  for (const cell of cells.filter((item) => item.samples >= 3 && item.expectancyR < 0).slice(0, 3)) {
    findings.push({
      priority: cell.performanceGate.state === "PAUSED" ? "high" : "medium",
      code: "negative_cell",
      evidence: `${cell.id} · ${cell.samples}笔 · Exp ${cell.expectancyR.toFixed(2)}R · PF ${cell.profitFactor == null ? "--" : cell.profitFactor.toFixed(2)}`,
      action: cell.performanceGate.state === "PAUSED" ? "该组合保持暂停，其他组合不受影响。" : "继续独立采样到门控样本量，不做全局降频。",
    });
  }
  const timeout = exits.find((item) => item.id === "timeout");
  if (timeout && timeout.samples >= 3 && timeout.expectancyR < -0.1) {
    findings.push({
      priority: "medium",
      code: "timeout_drag",
      evidence: `timeout ${timeout.samples}笔 · Exp ${timeout.expectancyR.toFixed(2)}R`,
      action: "把持仓时限/无进展退出作为下一轮实验变量，保持入场条件不变以隔离因果。",
    });
  }
  const stops = exits.find((item) => item.id === "stop_loss");
  if (stops && stops.samples >= 3 && stops.stopRecoveryRate != null && stops.stopRecoveryRate >= 0.4) {
    findings.push({
      priority: "medium",
      code: "stop_recovery",
      evidence: `止损后持续恢复率 ${(stops.stopRecoveryRate * 100).toFixed(0)}%（${stops.samples}笔止损）`,
      action: "优先验证止损是否过紧；只有后续观察继续确认时才调整结构止损。",
    });
  }
  if (overall.samples >= 5 && overall.earlyExitRate != null && overall.earlyExitRate >= 0.4) {
    findings.push({
      priority: "medium",
      code: "early_exit",
      evidence: `退出后被标记“退出偏早”的比例 ${(overall.earlyExitRate * 100).toFixed(0)}%`,
      action: "优先优化退出/移动止损，不增加新的入场过滤条件。",
    });
  }
