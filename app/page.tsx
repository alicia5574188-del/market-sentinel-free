  ...
  return <main>
    ...
    <section className="markets" hidden={tab !== "brain"}>{runtime?.symbols.map((symbol) => {
      ...
      return <article className="market" key={symbol}><div className="market-title"><div><small>{symbol.replace("_", "/")}</small><h2>{status}</h2></div><strong>{evidence?.midpoint ? num(evidence.midpoint, 5) : "—"}</strong></div>
        ...
        <div className="execution"><small>执行方式</small><b>{!marketFresh && plan?.state === "PREPARED" ? `原计划与边界保持不变；当前禁止成交，连续2份新盘口恢复后再核对，仍有效至 ${time(plan.expiresAt)}` : position?.status === "OPEN" ? waitReason(runtime, marketFresh, symbol) : plan?.state === "PREPARED" ? plan.marketState === "BREAKOUT" ? `内部实时确认 ${num(plan.entryTrigger, 5)}；强突破连续 ${(plan.breakoutSignalCount ?? 0)}/4，确认后IOC，0.5R外不追` : `价格到达 ${num(plan.entryTrigger, 5)} 后连续实时确认 ${(plan.realtimeSignalCount ?? 0)}/3；确认后IOC，不预挂交易所` : intent ? "方向已形成，等待系统建立实时进场计划" : reclaimedEdge ? "反弹和再次下破同时观察；只有实时证据确认的分支才IOC进场" : "继续等待完整机会"}</b></div>
    ...
    <section className="panel-list" hidden={tab !== "orders"}>{!openPositions.length && !preparedPlans.length && !recentClosedPositions.length && <div className="empty"><b>当前没有模拟订单</b><p>出现合适位置后会先显示准备计划；真实订单请进入底部「实盘」。</p></div>}
      ...
      {preparedPlans.map(({ symbol, plan }) => <OrderCard key={symbol} symbol={symbol} side={plan.side} label={runtime?.evidence[symbol]?.entryReady === false ? "计划冻结" : "等待实时确认"} state={plan.marketState} notional={plan.notional} equity={runtime?.equity ?? INITIAL_EQUITY} leverage={plan.leverage} margin={plan.margin} note={runtime?.evidence[symbol]?.entryReady === false ? `行情短暂延迟时不成交、不移动原边界；计划保留至 ${time(plan.expiresAt)}，连续2份新盘口后重新核对。` : (plan.invalidationSignalCount ?? 0) > 0 ? `软失效观察 ${plan.invalidationSignalCount}/2，计划仍锁定；连续确认后才撤销。有效至 ${time(plan.expiresAt)}。` : plan.marketState === "BREAKOUT" ? `${plan.routeKind === "INTERNAL_ROTATION" ? "子区间迁移" : plan.routeKind === "BREAKOUT_RETEST" ? "回踩再加速" : "父区间突破"}实时确认 ${(plan.breakoutSignalCount ?? 0)}/4；确认后IOC，0.5R外不追。有效至 ${time(plan.expiresAt)}。` : `价格到达观察位后实时确认 ${(plan.realtimeSignalCount ?? 0)}/3；确认后IOC，不向Gate预挂接刀单。有效至 ${time(plan.expiresAt)}。`} values={[[plan.marketState === "BREAKOUT" ? (plan.routeKind === "BREAKOUT_RETEST" ? "回踩再加速位" : plan.routeKind === "INTERNAL_ROTATION" ? "子区间观察位" : "父区间观察位") : plan.routeKind === "FAILED_BREAKOUT_REVERSAL" ? "反抽观察位" : plan.routeKind === "EDGE_REJECTION" ? "收回后观察位" : "实时观察位", plan.entryTrigger], ["扫盘外硬止损", plan.invalidation], ["实际第一目标", plan.target], ["计划风险", plan.plannedRisk]]} />)}
