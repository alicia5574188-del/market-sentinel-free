...
const OUTPUT = process.env.RESEARCH_OUTPUT ?? '/tmp/turnover-pair-relative-momentum.json';
...
const ENTRY_ZS = [1.00, 1.50, 2.00];
const PERSISTENCES = [0, 6, 12, 24];
const HOLDS = [6, 12, 24];
const DEPTHS = [1, 2, 3];
...
function pairFeature(a, b, t, window) {
  ...
  const r6 = intervalResidual(a, b, t, 6, beta);
  const r12 = intervalResidual(a, b, t, 12, beta);
  const r24 = intervalResidual(a, b, t, 24, beta);
  if (![r6, r12, r24].every(Number.isFinite)) return null;
  return { a, b, signalAt: t, window, corr, beta, z, absZ: Math.abs(z), r6, r12, r24 };
}
...
for (const window of WINDOWS) {
  ...
  for (const t of times) {
    if (new Date(t * 1000).getUTCHours() % DECISION_STEP_HOURS !== 0) continue;
...
function persistenceOk(f, hours) {
  if (!hours) return true;
  const r = hours === 6 ? f.r6 : hours === 12 ? f.r12 : f.r24;
  return Math.sign(r) === Math.sign(f.z) && Math.abs(r) > 1e-6;
}

function generate(config, until) {
  const byTime = featureByWindow.get(config.window); const busy = new Map(); const out = [];
  for (const [signalAt, rows] of byTime) {
    if (signalAt >= until) break;
    const entryAt = signalAt + HOUR; const exitAt = entryAt + config.hold * HOUR;
    if (exitAt > until) continue;
    const eligible = rows.filter((f) => f.corr >= config.minCorr && f.absZ >= config.entryZ && persistenceOk(f, config.persistence))
    ...
    for (const f of eligible) {
      ...
      // Continuation: long the beta-adjusted relative winner and short the relative loser.
      ...
      const gross = wA * aBase + wB * bBase; const adverseGross = wA * aAdv + wB * bAdv;
      out.push({ ...f, entryAt, exitAt, dirA, dirB, wA, wB,
        gross, base: gross - BASE_COST, stress: gross - STRESS_COST, adverse: adverseGross - BASE_COST,
        roundTripTurnover1x: 2, month: monthKey(entryAt) });
...
function stats(rows, from, to, field = 'stress') {
  ...
  return { events: xs.length, eventsPerDay: days ? xs.length / days : 0, turnover1xPerDay, net,
    avgDaily1000At1x: avgDaily * 1000, avgNet: xs.length ? net / xs.length : 0,
    pf: losses ? gains / losses : gains ? 99 : 0, winRate: xs.length ? xs.filter((e) => e[field] > 0).length / xs.length : 0,
    maxDrawdown, scaling };
}
...
for (const persistence of PERSISTENCES) for (const hold of HOLDS) for (const depth of DEPTHS) {
  const c = { id: `PRM-${id++}`, window, minCorr, entryZ, persistence, hold, depth };
  ...
  const oldPositiveMonths = positiveMonths(ev, OLD_FROM, OLD_TO); const currentPositiveMonths = positiveMonths(ev, CURRENT_FROM, TRAIN_TO);
  const sampleEnough = old.stress.events >= 120 && current.stress.events >= 90;
  const discoveryFloor = sampleEnough && old.stress.net >= 0 && current.stress.net >= 0
    && old.adverse.net >= 0 && current.adverse.net >= 0 && old.stress.pf >= 1.0 && current.stress.pf >= 1.0;
...
let evaluationOpened = false; let evaluation = [];
if (qualified.length) {
  ...
  for (const q of qualified.slice(0, 5)) {
    ...
    evaluation.push({ c: q.c, discoveryTurnover20: q.turnover20, discoveryMonthly20: q.monthly20,
      stress, adverse, positiveMonths: positiveMonths(ev, TRAIN_TO, EVAL_TO),
      evaluationPositive: stress.net >= 0 && adverse.net >= 0 && stress.pf >= 1.0 });
  }
}

const diagnostics = {
  candidates: candidates.length, diagnosticPool: diagnosticPool.length,
  ...
  discoveryQualified: qualified.length, evaluationOpened,
};

const output = {
  research: 'turnover-pair-relative-momentum-v1',
  objective: 'maximize genuine daily notional turnover; trade count is diagnostic only; after-fee stress non-loss is the floor; ~5% monthly is acceptable',
  periods: { old: [OLD_FROM, OLD_TO], currentTrain: [CURRENT_FROM, TRAIN_TO], evaluation: [TRAIN_TO, EVAL_TO] },
  protocol: { symbols, decisionEveryHours: DECISION_STEP_HOURS,
    construction: 'beta-weighted long/short correlated pair; follow rather than fade standardized cumulative residual; optional 6/12/24h residual-sign persistence',
    entry: 'next hourly open after completed signal', noFutureLeakage: true, noSelfTrade: true, noSameSymbolConcurrentPairs: true,
    costsPerUnitGrossNotional: { base: BASE_COST, stress: STRESS_COST, entrySlip: ENTRY_SLIP, adverseSlip: ADVERSE_SLIP },
    maxGrossX: MAX_GROSS_X, ddCaps: DD_CAPS,
    turnoverDefinition: 'round-trip sum of actual long+short gross notional; no fake/self-offset volume' },
  grid: { windows: WINDOWS, minCorrs: MIN_CORRS, entryZs: ENTRY_ZS, persistencesHours: PERSISTENCES, holds: HOLDS, depths: DEPTHS, candidates: candidates.length },
  diagnostics,
  topQualified: qualified.slice(0, 30), topNear,
  evaluation,
  note: 'This is the directional opposite of pair mean reversion: when a correlated pair develops a large beta-adjusted relative move, hold the relative winner long and loser short. The discovery floor is intentionally aligned to the revised turnover-first objective: both discovery eras must be non-negative after stress cost and adverse entry; no minimum trade-count or monthly-return target is imposed. The final 2026-06..2026-08 evaluation remains unopened unless that floor is passed.',
};
writeFileSync(OUTPUT, `${JSON.stringify(output)}\n`);
console.log(`TURNOVER_PAIR_RELATIVE_MOMENTUM=${JSON.stringify({ objective: output.objective, grid: output.grid, diagnostics, topQualified: output.topQualified.slice(0, 8), topNear: topNear.slice(0, 8), evaluation, note: output.note })}`);
