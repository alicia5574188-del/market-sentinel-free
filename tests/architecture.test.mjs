...
test("Forward Relation 2.0 is the only PAPER strategy authority and retired strategy stacks stay disconnected",async()=>{
  ...
  assert.match(core,/from "\.\/forward-family-probe\.ts"/);
  assert.doesNotMatch(core,/from "\.\/forward-entry-guard\.ts"/);
  const family=await read("lib/forward-family-probe.ts");
  assert.match(family,/FORWARD_FAMILY_PROBE_VERSION="forward-family-probe-v1"/);
  assert.match(family,/relationFamilyKey/);assert.match(family,/probeValueBlock/);assert.match(family,/familyAdmissionBlock/);
  assert.match(family,/STRUCTURE_STOP_NO_FEEDBACK/);assert.match(family,/MAX_NEW_PROBES_PER_5M=2/);
  assert.doesNotMatch(family,/PAPER_COST|TOTAL_RISK_RATE|SIDE_RISK_RATE|profitFloor|stopPrice|targetPrice|api\.bybit|api-futures\.kucoin/);
