...
const OUTPUT=process.env.RESEARCH_OUTPUT??'/tmp/passive-overshoot-fade-fast.json';
...
function rawTrades(config,scenarioName){
  const cacheKey=`${scenarioName}|${mechanismKey(config)}`;
  ...
  for(const ds of datasets){
    ...
    while(i<r.length-config.orderLifeBars-config.holdBars-2){
      ...
      const direction=z>0?-1:1,limit=Number(cur.close)-direction*config.entryAtr*atr;
      let fillIndex=-1;
      for(let j=i+1;j<=i+config.orderLifeBars&&j<r.length;j++){
        const bar=r[j],crossed=direction>0?Number(bar.low)<=limit*(1-scenario.fillBuffer):Number(bar.high)>=limit*(1+scenario.fillBuffer);
      ...
      for(let j=fillIndex+1;j<=last;j++){
        const bar=r[j],stopHit=direction>0?Number(bar.low)<=stop:Number(bar.high)>=stop,targetHit=direction>0?Number(bar.high)>=target*(1+scenario.fillBuffer):Number(bar.low)<=target*(1-scenario.fillBuffer);
  ...
  out.sort((a,b)=>a.openedAt-b.openedAt||Math.abs(b.signalZ)-Math.abs(a.signalZ));rawCache.set(cacheKey,out);return out;
}
function portfolio(trades,fraction){
  ...
  for(let i=0;i<trades.length;){
    const at=trades[i].openedAt;settle(at);const same=[];while(i<trades.length&&trades[i].openedAt===at)same.push(trades[i++]);
    for(const t of same){
      if(equity<=100||open.some(x=>x.symbol===t.symbol))continue;
      const notional=equity*fraction,plannedRisk=notional*(t.stopRate+0.0014),gross=sum(open.map(x=>x.notional)),risk=sum(open.map(x=>x.plannedRisk)),sideGross=sum(open.filter(x=>x.side===t.side).map(x=>x.notional));
      if(gross+notional>equity*1.5||risk+plannedRisk>equity*.10||sideGross+notional>equity*1.0)continue;
      const a={...t,equityAtOpen:equity,notional,plannedRisk,netPnl:notional*t.netReturnRate};open.push(a);accepted.push(a);
    }
  }
...
let selected=null;for(const row of discoveryCandidates){const v=row.stress.periods.validation;if(v.returnPct>0&&v.profitFactor>=1.0&&v.turnoverPerDay>=3.0&&v.turnoverPerDay<=7.0){selected=row;break;}}
