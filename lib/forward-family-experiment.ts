...
export type ActiveRecentValue={
  evidenceNetRate:number;targetRate:number;normalAdverseRate:number;retentionRate:number;samples:number;groups:number;
};
export function reserveExperimentValueBlock(input:{
  ...
  if(recent){
    ...
    if(recent.evidenceNetRate>=evidenceFloor&&pathRatio>=1.5&&recent.retentionRate>=.70
...
export function familyAdmissionBlock(input:{
  ...
  const activeRecent=input.rule.scope==="RECENT"&&input.rule.status==="ACTIVE"?{
    evidenceNetRate:input.rule.longNet,targetRate:input.rule.exitProfile.targetRate,
