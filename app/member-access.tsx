...
export function LoginGate({auth,onSession}:{auth:AuthSession|null;onSession:(s:AuthSession)=>void}) {
  ...
  return <main className="fr-app fr-access" data-access="login"><header className="fr-header"><div className="fr-brand"><span className="fr-emblem">↗</span><div><b>哨兵 · 关系引擎</b><small>PRIVATE ACCESS</small></div></div></header>
    <section className="fr-section"><div className="fr-section-head"><div><small>独立账户 · 同一策略源</small><h1>{owner?"主账户登录":"登录密钥"}</h1></div></div>
      ...
      <button className="fr-text-button" onClick={()=>{setOwner(v=>!v);setKey("");setError(null);}} disabled={busy}>{owner?"使用朋友登录密钥":"我是主账户所有者"}</button>
...
export function MemberAccess({auth}:{auth:AuthSession}) {
  ...
  if(!isOwner)return <section className="fr-section"><h2>我的使用资格</h2><p className="fr-note">账户：{auth.username}。你的登录密钥长期对应本账户，主账户生成其他朋友的密钥不会使它失效。你不能创建新用户，也不能查看其他会员的账户。</p><p className="fr-note">共享模拟订单源，不另起策略或重新冷启动。实盘由你自己配置和开启，仅复制开启后的新模拟单；成交价格与数量仍以你的Gate回报为准。</p></section>;
  return <section className="fr-section" data-testid="member-admin"><div className="fr-section-head"><div><small>仅主账户可见</small><h2>朋友与登录密钥</h2></div><span>{overview?.members.length??"—"} 位</span></div>
    <p className="fr-note">每次生成一位朋友的独立账户。发给朋友后点击“生成下一位”，旧密钥继续有效，包括尚未首次登录的朋友。不会重置任何交易账户。</p>
    ...
    <div className="fr-form"><label>朋友备注<input value={label} maxLength={60} onChange={e=>setLabel(e.target.value)} placeholder="例如：小王" disabled={busy}/></label>
    ...
    <div className="fr-member-list">{overview?.members.length?overview.members.map(m=><article className="fr-rule" key={m.id}><header><h3>{m.label}</h3><span>{m.activatedAt?"已激活":"待首次登录"}</span></header><p className="fr-note">{m.id}</p>
      <div className="fr-three"><div><small>本程序实盘成交额</small><b>{numberText(m.usage?.notional)} U</b></div><div><small>统计截至</small><b>{operatorTime(m.usage?.through)}</b></div><div><small>数据状态</small><b>{!m.usage?"等待本人配置":m.usage.error?"最近值·待核对":m.usage.partial?"部分已确认":"已核对"}</b></div></div></article>):<p className="fr-note">尚未发放登录密钥。这里不会显示朋友的余额、盈亏、密钥或实盘开关。</p>}</div>
  </section>;
}