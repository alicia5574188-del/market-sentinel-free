...
type Intelligence = {
  ...
  counterfactual: { trackedDecisionCount: number; maturedDecisionCount: number; status: "collecting"; note: string };
  portfolio: { directionConcentration: number; regimeSideConcentration: number; dominantFactor: string | null; riskState: "NORMAL" | "CONCENTRATED" | "HIGH" };
...
function relabelShell() {
  ...
  setText(radar?.querySelector<HTMLElement>(".utility-heading .eyebrow"), "Strategy 2.1 市场智能");
  setText(radar?.querySelector<HTMLElement>(".utility-heading strong"), "环境迁移 + 风险预警 + 学习状态");

  const orders = document.querySelector<HTMLElement>(".order-ledger");
  setText(orders?.querySelector<HTMLElement>(".utility-heading .eyebrow"), "Strategy 2.1 执行与学习");
  setText(orders?.querySelector<HTMLElement>(".utility-heading strong"), "组合风险 → Thesis → Execution → Learning");
  setText(orders?.querySelector<HTMLElement>(".calibration-title span"), "Strategy 2.1 决策校准");
  ...
  document.querySelectorAll<HTMLElement>(".lesson-card .section-title").forEach((section) => {
    setText(section.querySelector<HTMLElement>("span"), "本单复盘已进入 Strategy 2.1 学习记录");
    setText(section.querySelector<HTMLElement>("small"), "结构化归因继续进入经验矩阵");
  });
}
...
function StrategyPoolPanel({ pool, intelligence }: { pool: StrategyPool | null; intelligence: Intelligence | null }) {
  ...
  return <div className="strategy2-pool">
    <div className="strategy2-pool-head"><strong>12 Playbook 动态专家池</strong><span>Playbook {pool?.playbookCount ?? 0}/12</span></div>
...
function OpportunityCard({ item, intelligence }: { item: Opportunity; intelligence: Intelligence | null }) {
  ...
  return <div className={`v2-opportunity ${item.state.toLowerCase()}`}>
    ...
    {decision && <small className={`strategy21-decision-line strategy21-${decision.advisoryState.toLowerCase()}`}>Net EV {decision.netExpectedR == null ? "--" : `${decision.netExpectedR >= 0 ? "+" : ""}${decision.netExpectedR.toFixed(2)}R`} · 胜率估计 {(decision.estimatedWinProbability * 100).toFixed(0)}% · 决策置信 {decision.decisionConfidence} · 分歧 {decision.modelDisagreement} · OOD {decision.outOfDistributionRisk}</small>}
  </div>;
}
...
function OpportunityPanel({ packet }: { packet: Packet }) {
  ...
  return <div className="v2-panel v2-opportunity-panel">
    <div className="v2-panel-head"><div><span>Sentinel Strategy 2.1</span><strong>环境迁移 · 动态专家 · 概率 / EV 决策</strong></div><PermissionBadge market={market}/></div>
...
function SelectedAction({ opportunity, market, decision }: { opportunity: Opportunity; market: Market; decision: DecisionIntelligence | null }) {
  ...
  return <div className="strategy2-selected-action">
    ...
    {decision && <div className={`strategy21-selected-intelligence strategy21-${decision.advisoryState.toLowerCase()}`}><span>Net EV <b>{decision.netExpectedR == null ? "--" : `${decision.netExpectedR >= 0 ? "+" : ""}${decision.netExpectedR.toFixed(2)}R`}</b></span><span>胜率估计 <b>{(decision.estimatedWinProbability * 100).toFixed(0)}%</b></span><span>决策置信 <b>{decision.decisionConfidence}</b></span><span>专家权重 <b>{(decision.expertWeight * 100).toFixed(0)}%</b></span><span>模型分歧 <b>{decision.modelDisagreement}</b></span><span>OOD <b>{decision.outOfDistributionRisk}</b></span></div>}
  </div>;
}

function SelectedNext({ opportunity, decision }: { opportunity: Opportunity; decision: DecisionIntelligence | null }) {
  const baseItems = opportunity.state === "REJECT" ? opportunity.rejectReasons.map((reason) => REJECT[reason] ?? reason) : opportunity.state === "WATCH" ? opportunity.waitingFor : ["Strategy 2.0 已通过，等待组合风险、仓位与 Execution Engine 最终复核"];
...
function RadarPanel({ packet }: { packet: Packet }) {
  ...
  return <div className="v2-panel v2-radar-panel">
    <div className="v2-panel-head"><div><span>MARKET INTELLIGENCE · STRATEGY 2.1</span><strong>环境迁移与领先风险雷达</strong></div><PermissionBadge market={market}/></div>
    <MarketStrip market={market}/>
    <RegimeMigrationPanel intelligence={packet.intelligence}/>
...
function LearningPanel({ learning }: { learning: Learning | null }) {
  if (!learning) return <div className="strategy2-learning"><div className="strategy2-learning-head"><strong>Strategy 2.1 学习矩阵</strong><span>正在读取</span></div></div>;
  const recent = learning.recentTrades.slice(0, 4);
  const cells = learning.cells.slice(0, 5);
  return <div className="strategy2-learning">
    <div className="strategy2-learning-head"><strong>Strategy 2.1 学习矩阵</strong><span>Global × Asset × Playbook × Direction</span></div>
...
function LearningIntelligencePanel({ intelligence }: { intelligence: Intelligence | null }) {
  ...
  return <div className="strategy21-learning-intelligence">
    ...
    {intelligence.portfolio.dominantFactor && <small>主导风险因子代理：{intelligence.portfolio.dominantFactor} · {intelligence.portfolio.riskState}</small>}
...
function OrdersPanel({ packet }: { packet: Packet }) {
  ...
  return <div className="v2-panel v2-orders-panel">
    <div className="v2-panel-head"><div><span>PORTFOLIO + EXECUTION + LEARNING · STRATEGY 2.1</span><strong>账户风险、交易逻辑与学习治理</strong></div><PermissionBadge market={market}/></div>
...
export function Strategy2Dashboard() {
  ...
  return <>
    {targets.opportunity && createPortal(market && packet ? <OpportunityPanel packet={packet}/> : <div className="v2-panel v2-opportunity-panel"><div className="v2-panel-head"><div><span>Sentinel Strategy 2.1</span><strong>{packet?.error ?? "正在读取统一策略数据…"}</strong></div></div></div>, targets.opportunity)}
