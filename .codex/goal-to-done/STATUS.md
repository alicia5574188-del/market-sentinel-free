## V13.2 持续运行与五段真实走势叠加线图 — 本地实现中

- 已从当前历史路径PAPER开仓链移除三连亏冷却和当日亏损闩锁读取/写入；风险状态继续记录样本表现，但不再以PAUSED或0风险阻断新单。资金/保证金、单笔4%/组合12%、报价新鲜度、同币持仓、重复计划、D1与LIVE关闭保持。
- 顶部改为一张叠加线图：当前24根真实五分钟收盘线加最相似5段各36根真实五分钟收盘线；各自在参考点归零，历史虚线右侧为真实后一小时，不含平均线或预测线。聚焦回归test:direct 78项全绿，待完整门禁、一次PR/CI/部署和线上核验。

## V13.1 每单TP2净利至少30U、直接放大模拟仓位 — 生产完成

- 新历史路径PAPER订单单笔/组合风险改为4%/12%，取消会让第二笔及后续订单缩水的相关敞口折半；信号成立后按真实TP2距离反算并放大名义仓位，使每单TP2扣除估计往返成本后至少30U。30U不作为目标、上限或额外拒单条件，真实TP2不改，超过30U不截断；为表达该模拟仓位可使用流动性/波动/强平安全边界内的杠杆空间，所有持仓仍受12%组合计划止损约束。
- 不追改当前SOL等已开订单，不改变五段匹配、方向、0.8R路径经济边界、退出、brain/epoch、D1或LIVE关闭。本次按V13.1模拟资金政策补丁发布，避免为仓位变化强制归档持仓。
- PR #160/main `c29875a` 上线按真实TP2倒算放大仓位；线上复核后发现相关折半会让后续订单缩水，立即由PR #161/main `7a7a276` 移除。最终workflow `33988357115` verify/deploy/operations全绿，Worker `62a2c25b-fe04-4d1b-b0df-ba5e7d5a825a`；页面显示4%/12%与TP2至少30U，scanner live 6/6、position live、D1向外取整24h读1263348/写3016。当前最近三连亏触发原有30分钟暂停，暂无新单实盘快照；暂停解除后的新订单使用最终政策。

## V13 五段真实K线、频率与仓位修正 — 生产完成
...
- PR #158 已合并为main `86578d2`：匹配、方向、目标与回放统一只取最相似5段；每段携带并直接画24根匹配K线+12根实际后续K线。当前PAPER改为单笔1%/组合3%、相关敞口折半，日亏3%或三连亏暂停；经济门槛0.8R，2%保护及其余报价/流动性/资金/D1/实盘安全不变。新增0031隔离V13 PAPER证据，LIVE仍关闭。
- 本地test:direct 77、test:signals 233、build/UI 118及lint全绿。PR workflow `33984216762` 和最终PR workflow `33984543517` 全绿；真实12小时回放42信号、21完成、7胜、合计+2.891R，另2笔未完。BNB +3.700R、SOL +0.262R、DOGE -1.071R，BTC/ETH/XRP约0。该窗口支持提高频率但不证明未来盈利；不再继续放宽。
- main workflow `33984709869` verify/deploy/operations全绿；Worker `c69ee06f-08f1-4bfc-9f63-0d93870a419f`，0031 completed，active=`direct-market-brain-v13-five-analog-candles`、target=null、legacy open PAPER=0。线上页面显示当前24根和5段各36根真实K线、单笔1.00%；验收时SOL为60%先上并等待冻结回撤价103.7071。scanner/position均live、6/6分析、无错误；UTC日D1读929963/写2145，向外取整24h读1188918/写2880，低于安全线。
