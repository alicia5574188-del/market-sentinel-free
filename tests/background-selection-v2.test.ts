...
test("single-slot mode assigns exactly one symbol per invocation while preserving three-phase coverage", () => {
  const universe = [
    ticker("BTC_USDT", 0.12, 0.9),
  ...
  const selections = Array.from({ length: 12 }, (_, offset) =>
  ...
  assert.ok(selections.some((selected, offset) => offset % 3 === 1 && selected[0].symbol === "B_USDT"));
