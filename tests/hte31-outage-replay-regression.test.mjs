...
test("HTE31 outage recovery replays missed minute or legacy five-minute candles before normal quote management", () => {
  ...
  assert.match(recovery, /fetchGateMinuteRecovery/);
  assert.match(recovery, /evaluateScalpPosition/);
  ...
  assert.match(recovery, /observedAt: Math\.min\(now, candleStartMs\(candle\) \+ interval\)/);
