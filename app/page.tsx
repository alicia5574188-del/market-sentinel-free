...
export default function Home() {
  ...
  return <main>
    ...
    <section className="markets" hidden={tab !== "brain"}>{runtime?.symbols.map((symbol) => {
      ...
      const status = !authorityOperational ? "后台循环恢复中" : !evidence?.fresh ? plan?.state === "PREPARED" ? "计划冻结 · 行情延迟" : "行情短暂延迟" : !evidence.ancillaryFresh ? "关键结构刷新中" : evidence.entryReady === false ? `恢复确认 ${evidence.recoveryFreshCount ?? 0}/2` : position?.status === "OPEN" ? "持仓中" : plan?.state === "PREPARED" ? "等待进场" : intent ? "发现机会" : evidence?.warmup < 30 ? `预热 ${evidence?.warmup ?? 0}/30` : "继续观察";
