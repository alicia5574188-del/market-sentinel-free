...
- Legacy physical count/boundary drift is accepted only if every row passes the
  ...
  existing normalization rules at the account's original persisted evidence
  time reconstruct the exact authenticated page bytes. Startup time must not be
  used for this proof because normal 24-hour aging would remove old rows before
  validation. New raw-hash mismatch or different persisted-time canonical bytes
  remains fail-closed. Per-page shortages are allowed only as redistribution
  inside the same authenticated whole; total shortage is rejected.
...
Version region-launch-v3 fences old pending signals but retains mother failure/consumption memory and every open trade's original lifecycle. Reuse existing Gate dual transport and quote/data cadence; no Worker method hashes change. Only the user-authorized forward source fingerprint advances for passing existing paths/quotes to the new pure evaluator and version guard. Details and limitations:research/LAUNCH_FIVE_MINUTE_2026-09-23.md.
...
Explicitly replace the obsolete55k owner-only write plan with scoped63,032 primary /99,192 two-member reserved rows. No billing change; request/duration/unbounded workload capacity is not certified. Keep new storage reader during rollback, or atomically rewrite fully restored legacy state first. Only two frozen primary methods change (advanceForwardNow/saveCheckpoint); eight others stay frozen. Record same-input correctness superiority separately from unverified profit. After local and exact-head remote gates, authorized main deployment requires a public advancing-state/identity/intent receipt.

# 2026-09-20 — hold the audited repair, do not deploy (historical intermediate decision)
...
# 2026-09-12 — Risk safety must reject economically meaningless PAPER size
...
# 2026-09-12 — reject disproven 3× sizing; ship immediate protection only

- The published 10%–12% drawdown came from an old fixed-0.5× research portfolio and cannot justify tripling the current 1×–4× production sizing. Production-equivalent 3× tests lost money in at least one chronological segment and reached roughly 34%–58% drawdown; the fixed-1.5× interpretation also missed the training profitability gate.
