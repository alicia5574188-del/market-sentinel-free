...
export const hte31ShadowSamples = sqliteTable("hte31_shadow_samples", {
  ...
  sampleKind: text("sample_kind", { enum: ["near_ready", "ready"] }).notNull().default("near_ready"),
  playbookId: text("playbook_id").notNull().default(""),
  maxHoldingMinutes: integer("max_holding_minutes").notNull().default(240),
  confidence: integer("confidence").notNull().default(0),
  setupScore: real("setup_score").notNull().default(0),
  ...
  terminalPrice: real("terminal_price"),
  terminalReason: text("terminal_reason", { enum: ["stop_loss", "take_profit", "timeout"] }),
  ...
  index("hte31_shadow_kind_status_idx").on(table.sampleKind, table.status, table.entryAt),
]);
