...
export type LiveStrategyTradeLink = {
  source: "hte31" | "legacy";
  id: string;
  status: string;
...
async function requireHte31LiveIntentSource(tradeId: string) {
  const [hte] = await getDb().select({ id: hte31Trades.id }).from(hte31Trades).where(eq(hte31Trades.id, tradeId)).limit(1);
  if (!hte) throw new Error("HTE 3.1 实盘候选已不存在，禁止创建 Gate 订单");
  return hte;
}

/**
 * Resolve the strategy lifecycle behind a persisted live order.
 *
 * New HTE31 live orders point directly at `hte31_trades` using the existing
 * `live_orders.trade_case_id` lineage key. The column name is intentionally kept
 * unchanged so already-persisted Gate orders remain readable without a risky D1
 * migration. Legacy `trade_cases` are read only as a historical recovery fallback
 * for live orders created before HTE31 became the sole new-entry authority.
 */
export async function getLiveStrategyTradeLink(tradeId: string): Promise<LiveStrategyTradeLink | null> {
  const db = getDb();
...
export async function createLiveOrderIntent(values: typeof liveOrders.$inferInsert) {
  const db = getDb();
  // New live intents must have a real HTE31 source row, but no compatibility
  // copy is written into the retired `trade_cases` ledger.
  await requireHte31LiveIntentSource(values.tradeCaseId);
