当前最高优先修正：V12解决页面把部分币种429/超时误报为全系统异常，以及V11仍被历史期望/目标命中率/净亏比例重复拦截而长期不开单。多数独立历史片段的途中首段方向直接授权模拟入场；路径统计用于保护、目标和学习，不再第二次否决方向。保留新鲜报价、入场区、成本覆盖、2%历史保护距离、0.25%单笔/0.75%组合风险、亏损暂停和实盘关闭。完成本地全回归、CI真实12小时回放、部署、PAPER切换和线上订单/状态验证。
