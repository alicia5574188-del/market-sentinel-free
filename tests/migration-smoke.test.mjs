...
test("纠正迁移隔离旧观察记录且不改写真正的合约持仓", () => {
  ...
  applyMigration(db, "0005_many_tenebrous.sql");
  const credentialColumns = db.prepare("PRAGMA table_info(live_exchange_credentials)").all().map((row) => row.name);
  assert.ok(credentialColumns.includes("ciphertext"));
  assert.ok(credentialColumns.includes("iv"));
  assert.ok(!credentialColumns.includes("api_key"));
  assert.ok(!credentialColumns.includes("api_secret"));
  ...
  const liveControl = db.prepare("SELECT entry_enabled, state, activation_epoch FROM live_trading_control WHERE id = 1").get();
  assert.deepEqual({ ...liveControl }, { entry_enabled: 0, state: "disabled", activation_epoch: 0 });
  applyMigration(db, "0006_aberrant_harry_osborn.sql");
  const liveRiskColumns = db.prepare("PRAGMA table_info(live_trading_control)").all().map((row) => row.name);
  assert.ok(liveRiskColumns.includes("account_equity_peak_usdt"));
  assert.ok(liveRiskColumns.includes("daily_realized_pnl_usdt"));
  assert.ok(liveRiskColumns.includes("account_risk_checked_at"));
  applyMigration(db, "0007_dapper_meggan.sql");
  const ownerColumn = db.prepare("PRAGMA table_info(live_exchange_credentials)").all().find((row) => row.name === "owner_account_id");
  assert.equal(ownerColumn.notnull, 0);
  assert.equal(db.prepare("SELECT owner_account_id FROM live_exchange_credentials WHERE id = 1").get().owner_account_id, null);
  applyMigration(db, "0008_easy_ezekiel.sql");
  const liveOrderColumns = db.prepare("PRAGMA table_info(live_orders)").all();
