...
function signalRiskMultiplier(signal: Strategy2Signal) {
  const metric = signal.metrics.find((item) => item.key === "v2-risk-multiplier");
  return metric && Number.isFinite(metric.score) ? Math.max(0, Math.min(1, metric.score)) : 1;
}
...
function growthPacket(packet: GateAnalysisPacket, signal: ReadyGrowthSignal): GateAnalysisPacket {
  ...
  return {
    ...
    decision: {
      ...packet.decision,
      state: "confirmed",
      stateLabel: "Strategy 2.0 确认",
      side: signal.side,
      confidence: signal.confidence,
      directionalScore: signal.score,
      posteriorLong,
      regime: regimeKey,
      action: `${signal.label}触发，按 ${signal.strategyMeta.tradeMode ?? "standard"} 风险执行`,
      thesis: `Sentinel Strategy 2.0 当前由「${signal.label}」主导。${supporting.length ? `同向支持：${supporting.join("、")}。` : ""}${signal.thesis}`,
      entryZone: signal.entryPlan.entryZone,
      trigger: `Strategy 2.0 · ${signal.label}：${signal.reasons.join("；")}`,
      ...
      diagnostics: { ...packet.decision.diagnostics, confirmationCount: signal.reasons.length, atrPct: signal.regime.atrPct ?? packet.decision.diagnostics.atrPct },
    },
  };
}

export async function retireLegacyShadowTrades() {
  const archived = await getDb().update(tradeCases).set({ activeKey: null, status: "archived", archivedAt: Date.now(), learningApplied: true }).where(and(
...
export async function getStrategyLabDashboard() {
  const rows = await getDb().select({
    ...
    regime: tradeCases.regime,
  }).from(tradeCases).where(eq(tradeCases.simulationModel, "contract_v2")).orderBy(desc(tradeCases.entryAt)).limit(2500);
  ...
  return {
    observedAt: Date.now(),
    note: "Strategy 2.0：12 套 Playbook 同时评估、竞争与学习；同一币只建立一个主仓位，多策略同向作为支持 Thesis。",
    executionGovernor: { state: governor.state, reason: governor.reason },
    baseline: { id: "baseline_v1" as const, label: "Sentinel Strategy 2.0", mode: "baseline" as const, openCount: rows.filter((row) => row.status === "holding").length, stats },
    strategies: STRATEGIES.map((strategy) => ({
      ...
      promotion: {
        ...
        reasons: ["所有 Playbook 从第一天参与评估；真实交易按环境、证据和探索价值竞争主仓位"],
      },
    })),
  };
}
